...
OWNER_ACTIONS:
  ...
  items:
    ...
    - id: lifecycle-email-migration
      title: "Schedule the H14/H15 + H16–H18 (trial T1–T3) lifecycle crons + email provider key (migration 0019 already APPLIED to prod via MCP; no new migration for T1–T3 — preference_signals.topic + lifecycle_email_sends.email_type are free text)"
      priority: high
      status: open
      why: "H14 (month-3 annual nudge) + H15 (win-back), PR #221, AND the new H16–H18 trial-ending sequence (T1 welcome / T2 ~2-days-left / T3 expiry-day annual-upsell, PR #658, run 95) all persist one row per (user, campaign) in `lifecycle_email_sends` (RLS tenant-isolation, grocery_app + app_current_user_id() + explicit GRANT) so a user is never re-emailed for the same campaign. The trial (T1–T3) queries read the `subscription_status` + `trial_end_at` preference-signals the Stripe webhook now writes — so trial emails only work once the webhook is live in prod (already is) AND a provider is connected. All campaigns stay DORMANT until an email provider key is set (sends dry-run-skip + are NOT recorded — no fake success) AND the cron routes are scheduled. No adoption % is banked in the business case — these are dormant infra until you connect + schedule them. NOTE (known minor follow-up, tracked in LOOP_MEMORY run 95): a user who cancels DURING the trial can still receive T2/T3 until trial end — low severity (copy says 'cancel anytime, no charge'); a future refinement will record cancel_at_period_end and exclude them."
      how: "1) Run `pnpm --filter @gm/db db:migrate` (idempotent; 0019_lifecycle_email_sends.sql — no NEW migration for T1–T3). Verify: `SELECT * FROM lifecycle_email_sends LIMIT 1;` returns empty (table exists). 2) Set an email provider key (RESEND_API_KEY / SENDGRID_API_KEY / POSTMARK_API_KEY — first found wins) + EMAIL_FROM / EMAIL_UNSUBSCRIBE_SECRET (see track-h-activation). 3) Schedule ALL FIVE routes as Vercel Cron Jobs, passing the secret (they are already declared in vercel.json crons; confirm the deploy picked them up): `/api/cron/h14-annual-nudge?key=$CRON_SECRET` (weekly), `/api/cron/h15-winback?key=$CRON_SECRET` (weekly), and — DAILY, since the trial window is time-sensitive — `/api/cron/h16-trial-welcome?key=$CRON_SECRET` (08–11:00), `/api/cron/h17-trial-reminder?key=$CRON_SECRET` (12:00), `/api/cron/h18-trial-expiry?key=$CRON_SECRET` (early, 08:00). Vercel Hobby allows daily-granularity crons. Until both steps are done the routes return {sent:0, skipped:N} honestly."
      blocks: none
