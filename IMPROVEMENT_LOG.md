...
## 2026-07-25 (run 94) — DEEP AUDIT (standalone 4-Haiku lens sweep, ~24h since run-92's 7-lens) + 3 file-disjoint clears (#648 use-it-up 44px, #649 staples 44px, #650 normalizer prompt-injection hardening); 2 abandoned on Reviewer-B value rejections; 0 circuit-breaks

Baseline green at run start (typecheck 6/6; 1067 `@gm/core` tests +27 skip; production `next build` exit 0, no missing-export; mobile `npm ci` + `tsc` clean; validation 8/8 `--readiness` READY 0 unmet; prices 499/3999/999/7999¢ byte-consistent; SUMMARY base $33,450 / floor_met false; 0 open PRs; clean tree). A standalone DEEP AUDIT was due (~24h since run-92's 7-lens), so ran a **4-Haiku read-only lens sweep over the whole repo** BEFORE scouting: security/RLS/Track-G · correctness/dead-code · design/a11y/taste · perf+coverage+artifact-freshness.

**Shipped (3, file-disjoint) — all first-pass 2/2 APPROVE:**
- **#648 — `a11y(use-it-up)`: raise the "Tossed it" waste action to the 44px touch target.** The waste-logging button in the "Expiring soon" list was `btn-sm` (~28-30px), under the WCAG 2.5.5 / Apple HIG 44px bar. It's the right-flush action in a `.row`, so it uses the established negative-margin idiom `min-h-[44px] -my-2 -mr-2` (grows the hit area without reflowing the row; matches `pantry/page.tsx`). Part of the systematic 44px program.
- **#649 — `a11y(staples)`: raise the dose "Set" + autopilot toggle to 44px.** Both staples-autopilot controls were `btn-sm` (~28-30px). "Set" sits mid-row → `min-h-[44px] -my-2` (net-zero height); the autopilot toggle is in a `.row items-start` whose taller left column governs row height, so `min-h-[44px]` alone can't reflow (applied to both the enabled and disabled `className` branches). Daily-use controls on the depletion-tracking feature.
- **#650 — `harden(ingestion)`: JSON-encode the untrusted receipt name in the normalizer prompt [Track G].** `createLlmNormalizer` is fed receipt-line text from **Gmail-ingested email** (untrusted, externally supplied via `gmail-sync.ts:175`), previously interpolated raw as `` `"${name}"` `` into the LLM prompt — an indirect prompt-injection surface (OWASP LLM01-class). The `matchId` verifier (rejects any id not in the candidate set) already bounds the blast radius, so this is **defence-in-depth, not the hard guarantee**; but never raw-interpolate untrusted text into a prompt — switched to `${JSON.stringify(name)}`. Added a regression test with a **capturing fake client** that neutralizes a crafted quote-breaking injection string (asserts against the REAL constructed prompt, so it can't pass by accident). Full `@gm/core` suite green (1072 pass). Reviewer A verified the untrusted-vs-trusted scoping (candidate names, DB-sourced, correctly left as-is); Reviewer B verified the `gmail-sync` call site makes `name` genuinely externally-supplied.

**Abandoned (2) — both Reviewer-B value rejections, clean, no rework (the maker≠checker guard filtering cheap-scout over-reports):**
- **media/models resolver test (`packages/core/src/media/models.test.ts`) — DUPLICATE, zero net coverage.** The Haiku perf/coverage lens flagged `resolveImageModel`/`resolveMusicModel` as "previously untested," but Reviewer B caught that the **identical** assertions already live in `media-gen.test.ts:33-52` — and that file is the **capabilities-manifest keyless proof** for `marketing-media-gen` in `packages/config/capabilities.json`. Verified: `media-gen.test.ts` asserts the same quality-band → pinned-id mappings against the same `MEDIA_MODELS`. A second file would be pure padding. Abandoned.
- **capture.tsx `added` DTO guard — SPECULATIVE, no live drift.** The correctness lens flagged that a drifted/absent `added` could render "Added undefined items…". Reviewer B verified `/api/mobile/capture` → `captureToList` (`packages/core/src/capture/add.ts`) returns `added: number` on **every** path — never optional. Unlike the real observed DTO mismatches the loop actually fixed (#638 dead-on-arrival pantry list, #642 raw slugs), there's no current or announced drift to guard against — a harmless one-liner dressed as a bug fix. Abandoned per the value bar.

**Deep-audit findings noted but NOT actioned this run (with evidence):**
- **SECURITY — CSP allows `'unsafe-inline'`/`'unsafe-eval'` in `script-src` (HIGH-labeled).** A genuine hardening opportunity, but a nonce-based CSP is cross-cutting (per-request middleware nonce + forces dynamic rendering) and conflicts with this repo's careful static-`/404` handling (CLAUDE.md); the theme inline script in `layout.tsx` likely *requires* `unsafe-inline`, so removal is unverifiable in a headless env. **DEFERRED as too risky to attempt blind** — a future run should confirm the inline theme script can carry a nonce, browser-verify the theme still applies AND the CSP enforces, then migrate. Not a live tenant/auth hole; the scorecard already grades security A with strong headers (HSTS/nosniff/frame-ancestors).
- **SECURITY — multi-instance in-memory rate-limit/quota/demo-ceiling.** KNOWN, owner-gated (Upstash Redis), already tracked in PENDING_OPS + the scorecard's standing A→A+ nit. CORS `Allow-Methods`/`Allow-Headers` without `Allow-Origin` = the secure same-origin default, NOT a hole.
- **PERF — `getActiveListView` sequential awaits.** Marginal (postgres.js pipelines statements on one tenant connection; non-latency-critical mobile read) → deferred.
- **ARTIFACTS — CLEAN.** README / BUSINESS_CASE / pricing / store copy all consistent with code (prices 499/3999/999/7999¢; ~33% annual-savings math checks out).

**Lessons:** (1) **A cheap Haiku scout will confidently call a pure module "previously untested" — ALWAYS grep for an existing sibling/barrel test (and cross-check `capabilities.json` for a registered keyless proof) before treating a "coverage gap" as real.** The media/models finding looked clean (no adjacent `models.test.ts`), but the assertions already lived in `media-gen.test.ts`, the capability's proof file. (2) **Distinguish a defensive DTO guard for an *observed* drift (real value — #638/#642 fixed actually-broken screens) from a guard for a boundary that returns the right shape on every path (padding).** Both traps were caught by the Sonnet value reviewer before merge — the guard doing its job, not churn.

**Monetization: RE-CONFIRMED reach-gated** — no buildable, non-reach-dependent revenue lever (every pricing/tier/conversion/retention/margin lever built; base ≈ $33K < $100K floor = owner-GTM #190, ~12th consecutive run). **Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor; Confidence UNCHECKED). Validation 8/8, 0 unmet; ship gate MET (A). **Security/RLS/Track-G re-confirmed no new holes** by the deep audit. A productive convergent run — the DEEP AUDIT earned its keep (3 real clears incl. a genuine security hardening), and the 2 abandons are the maker≠checker guard correctly filtering cheap-scout over-reports.

## 2026-07-18 (run 83) — DEEP AUDIT (standalone 6-Haiku lens sweep, due since run 77) + 3 file-disjoint clears (#605 mobile gmail-perk store-honesty, #606 billing env-var docs, #607 cook decimal-scale coverage); 6/6 Sonnet approve first-pass; 0 abandons, 0 rejections

Baseline green at run start (branch level with main; tree clean; validation 8/8 `--readiness` READY 0 unmet). Last standalone/folded deep audit was run 77 (2026-07-16); runs 78-82 were scout sweeps, so a standalone DEEP AUDIT was due (~5 runs / >24h). Ran a **6-Haiku read-only lens sweep over the whole repo**: security/RLS/Track-G · correctness/functional/uncaught-throws · Track-F coverage · design/a11y/taste · artifact-freshness/monetization · perf/mobile-parity. **2 lenses CLEAN (security, correctness)**; the other four produced findings I verified before selecting — shipping 3, rejecting the rest as anti-pattern / redundant / marginal.

**Shipped (3, file-disjoint):**
- **#605 — `fix(mobile)`: don't advertise Gmail receipt import on the paywall [store-acceptance honesty].** `apps/mobile/app/upgrade.tsx` mapped `PREMIUM_PERKS` and filtered only `household`, leaving **"Gmail receipt import"** advertised, while the native app has **no** Gmail-OAuth surface (`grep -rin gmail apps/mobile` → zero; capture is snap/quick-add only). Filtered `gmail_import` out alongside `household` via a small `MOBILE_HIDDEN_PERKS` set. Mobile typecheck green; both Sonnet reviewers APPROVED (perk verified unreachable natively; header subtitle stays accurate). **⚠️ REVERSAL OF A TWICE-AFFIRMED DELIBERATE DEFERRAL — see the process note below.** This reverses run 80's recorded judgment (IMPROVEMENT_LOG run-80 line: "`gmail_import` is a **live** feature — a premium user connects Gmail on the web and the synced pantry benefits their shared account on mobile, so the value IS deliverable; filtering it would understate real value") which run 82 re-affirmed. Run 80 framed the future options as "filter vs. build a deep-link-to-web connect path" — so filtering was an explicitly-named legitimate option, not forbidden. **Why the reversal stands:** the mission's binding constraint is store acceptance at HIGH confidence (a hard launch gate). An App Store reviewer tests the mobile binary in isolation and has no in-app path to reach Gmail import, so advertising it is a real Apple 2.3.1 rejection vector; the value cost of dropping one of six perks (the app still lists AI planner, unlimited Discover, recipe remix, spend insights, wrapped+) is marginal, and the cross-platform-account argument doesn't survive isolated review. The filtered state is strictly safer on the binding constraint at a marginal value cost, so it is the better state for the mission. The decision is now settled the OTHER way, **with engagement** (not as a fresh oversight).
- **#606 — `docs(ops)`: document the billing env vars in `.env.example` + `OPERATIONS.md` [LIVING ARTIFACTS / operability].** `.env.example` had **no** billing section at all — the one subsystem the setup template silently dropped — while `packages/config/src/env.ts` validates the full Stripe + RevenueCat set. `OPERATIONS.md`'s env table listed five of seven billing vars but omitted `STRIPE_PRICE_FAMILY` and `REVENUECAT_WEBHOOK_AUTH`, both load-bearing: `/api/stripe/checkout` reads `STRIPE_PRICE_FAMILY` for Family checkout (503 if unset), `/api/webhooks/revenuecat` fails closed (401) on every entitlement webhook without `REVENUECAT_WEBHOOK_AUTH` — so a paying mobile user's premium would silently never land. Added the Billing section (empty placeholders + role comments mirroring env.ts) + the two rows. No secrets committed; both reviewers verified names/roles/prices against env.ts and the live routes.
- **#607 — `test(recipe)`: cover scaleMeasure's off-fraction decimal fallback [Track F].** `scaleMeasure` (cook mode's ×1/×2/×3 scaler, `cook-mode.tsx:119`) renders via `formatQty`, which snaps to common fractions (⅛…⅞ within 0.02) else `return String(rounded)` (cook.ts:60) — the **only** uncovered line in `cook.ts` (all prior tests used integer/common-fraction results). Real ingredient measures aren't round ("1.1 cups"×2→"2.2"), so the fallback is reachable + user-facing. Added one case (three assertions incl. the whole-part-0 path) → `cook.ts` 100% line coverage. Full `@gm/core` suite green (1043 passed). Both reviewers hand-traced the float math (incl. `0.7*3 = 2.0999… → 2.1`).

**Deep-audit findings REJECTED after verification (not shipped):**
- **Design a11y — recipe images `alt=""` across 9 pages → `alt={title}` (REJECTED as an a11y anti-pattern).** In every one of those layouts the recipe title is rendered as adjacent/overlaid text (`{r.title}` in the card, the `<h1>` over the share-page hero). Empty alt is the CORRECT choice for images redundant to adjacent text; `alt={title}` would double-announce the title to screen readers. This is the same class of Haiku false positive the loop has rejected before (the recurring `alt=''` scout trap, runs 42-47). Verified the site context before rejecting.
- **Coverage — reconcile.ts confidence-dedup (lines 205/217), onboarding cleanOptions cap, semantic-layer plan_my_week degrade, ingest idempotency (4 candidates, DEFERRED as marginal/redundant).** The reconcile higher-confidence-dedup pattern is already covered on the sibling confirmations branch (line 100-104); the others are low-value cap/degrade paths or require heavy harness for <1 line delta. Shipping them would be padding — cut per the value bar.
- **Monetization — RE-CONFIRMED reach-gated.** No buildable, non-reach-dependent revenue lever found; every pricing/tier/conversion/retention lever is built. Base ≈ $33K < $100K floor = owner-GTM #190.

**Process note / lesson (the gmail_import reversal):** the perf/mobile scout re-surfaced `gmail_import` as a "new" store-honesty bug, and I shipped it as a fresh oversight WITHOUT first cross-checking the ledger for the specific prior deferral (I checked the household/Family *pattern* but not the `gmail_import` *decision*). PR #605 auto-merged before I found run 80's recorded judgment. The outcome is defensible (and arguably the better state — see #605 above), but the process slipped: **a scout finding on a known-deferrable surface must be cross-checked against the ledger's specific prior decision BEFORE arming auto-merge, not after** — and the scout's "already-settled" list should have named `gmail_import` explicitly. Recorded in LOOP_MEMORY + LOOP_HEALTH so the reversal is now a documented, engaged decision rather than an unacknowledged flip.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM; every buildable revenue lever built). Confidence statement stays UNCHECKED. Validation 8/8, 0 unmet. **Security/RLS/Track-G + correctness/functional re-confirmed CLEAN** by the deep audit. A DEEP-AUDIT run with 3 real clears (1 store-honesty, 1 living-artifact, 1 Track-F) + a documented decision reversal = success.

## 2026-07-17 (run 82) — Converged run: 5-Haiku scout sweep (folded deep-audit-equivalent) + 2 file-disjoint clears (#602 manage-subscription store-honesty, #601 install-prompt 44px a11y); 4/4 Sonnet approve first-pass; 0 abandons, 0 rejections

Baseline green at run start (typecheck clean all 6 projects, 1036 core tests + 27 skipped, self-validation 8/8 READY 0 unmet, prod build exit 0 with no missing-export warnings, 0 open PRs, clean tree). Run 77 folded a standalone deep audit ~24-30h earlier; this run's 5-Haiku read-only scout sweep across the deep-audit lenses (Track-F coverage · security/RLS/Track-G · design/a11y + living-artifact · correctness/mobile-parity · monetization/store-honesty) doubled as a folded deep-audit-equivalent. **3 of 5 lenses NO-FINDINGS** (security/RLS/Track-G, Track-F coverage, correctness/mobile-parity — all re-confirmed CLEAN).

**Shipped (2, file-disjoint):**
- **#602 — `fix(billing)`: gate household sharing + Family tier on manage-subscription [store-acceptance honesty / LIVING ARTIFACTS].** `apps/web/app/manage-subscription/page.tsx` — a reachable free-user page linked from `/profile` — advertised **"household sharing"** in its "Upgrade to Premium" CTA copy AND rendered the **Family** (`premium_family`) plan card **unconditionally** in its plans grid, while `FEATURE_HOUSEHOLDS` defaults **off**. It was the **last un-gated surface** after runs 78-81 gated the mobile paywall (#594), the landing page, `/upgrade`, and `/help` (#596). The Family card was easy to miss because the grid's price display is tier-specific (only `premium_monthly`/`premium_annual` render a price), so `premium_family` silently rendered as a bare "Family" label with no price — still advertising a dark tier (Apple 2.3.1 / Google accurate-listing risk). Mirrored the `/upgrade` gate: imported `householdsEnabled` from `@gm/db`, computed `const householdsLive = householdsEnabled()`, gated the perk copy (`Gmail import{householdsLive && ", household sharing"}, and more.`), and filtered the plans grid (`SUBSCRIPTION_PLANS.filter((p) => p.tier !== "free" && (householdsLive || p.tier !== "premium_family"))`). Both reviewers confirmed the `{householdsLive && …}` conditional renders no stray "false" (React omits booleans), the sentence is grammatical in both flag states, and the section is `!isPremium`-only (no premium-user regression). `householdsEnabled()` is a pure `process.env` read (no DB) — safe in the server component. Web typecheck + production `next build` exit 0, no missing-export warnings.
- **#601 — `fix(a11y)`: enlarge the install-prompt dismiss button to a 44px tap target.** The PWA install-prompt (`apps/web/app/components/install-prompt.tsx`) dismiss button was `btn-ghost btn-sm` (`px-3 py-1.5` + an `h-4` icon ≈ **28px** tall), below the repo's 44px touch-target bar. Switched it to the established banner-dismiss idiom `flex h-11 w-11 items-center justify-center rounded-lg text-ink-400 transition-colors hover:bg-ink-50 hover:text-ink-700` — the same pattern `gmail-upgrade-banner.tsx` (#584) and `theme-toggle.tsx` already use — with a neutral `hover:bg-ink-50` (install-prompt is a neutral surface, not brand-tinted) and `aria-hidden` on the decorative `X` (the button keeps its `aria-label`). install-prompt is a spacious floating card banner (the same layout class as the Gmail banner), so the visible `h-11 w-11` idiom is the correct precedent here, not the negative-margin hit-slop used for dense grid controls. Both reviewers confirmed `h-11 w-11` = a true 44px target, the `items-center` row grows without clipping/misalignment, and `hover:bg-ink-50` is a valid token. Web typecheck + production `next build` exit 0, no missing-export warnings.

**Scout lenses that returned NO clean actionable gap (verified):**
- **Track-F coverage** — the coverage scout swept ~40 `packages/core/src` files (recipe scoring, reorder/par, pantry depletion/EWMA, units, nutrition, personalization, experiments/stats, ingestion, capture, billing, security) and, after verifying each function against BOTH sibling `*.test.ts` and barrel test files, found NO clean mutation-sensitive reachable pure-logic gap — the ~1036-test suite is genuinely converged.
- **security/RLS/Track-G** — CLEAN (all public tables through 0021 RLS+policy [tenant_isolation / catalog_access→grocery_app / admin]; ~41 routes rate-limited + zod-validated + error-hygienic; login lockout 10→15min timing-safe no-enumeration; per-user LLM quota + demo per-IP/global spend ceiling; Turnstile fail-open-loud; Stripe `constructEvent` + timing-safe RevenueCat/Gmail/cron webhooks; CSP/HSTS/nosniff/Permissions-Policy; entitlements server-side; no committed secrets).
- **correctness/mobile-parity** — CLEAN (critical paths try/catch + LLM `withTimeout` < serverless budget; no dead code/TODO debt on live paths; mobile paywall perks all map to reachable native screens — `household` intentionally dark-filtered, `gmail_import` a deliberate deferred judgment call from run 80).

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM; every buildable revenue lever built). Confidence statement stays UNCHECKED. Validation 8/8, 0 unmet. **Security/RLS/Track-G re-confirmed CLEAN.** A coherent converged run that closed the last dark-FEATURE_HOUSEHOLDS store-honesty gap + an a11y tap-target clear = success.

## 2026-07-17 (run 81) — Converged run: 4-Haiku scout sweep (folded deep-audit-equivalent) + 3 file-disjoint clears (#596 help store-honesty, #597 + #598 Track-F coverage); 6/6 Sonnet approve first-pass; 0 abandons

Baseline green at run start (typecheck clean all 6 projects, 1036 core tests + 27 skipped, self-validation 8/8 READY 0 unmet, prod build exit 0 with no missing-export warnings, 0 open PRs, clean tree). Run 77 folded a standalone deep audit ~24h earlier; this run's 4-Haiku read-only scout sweep across the deep-audit lenses (security/RLS/Track-G · design/a11y + living-artifact/pricing · Track-F coverage · correctness/mobile/dead-code) doubled as a folded deep-audit-equivalent. **Security/RLS/Track-G lens NO-FINDINGS** (re-confirmed CLEAN — see below).

**Shipped (3, file-disjoint):**
- **#596 — `fix(help)`: gate household-sharing docs behind FEATURE_HOUSEHOLDS [store-acceptance honesty / LIVING ARTIFACTS].** The Help & FAQ page (`apps/web/app/help/page.tsx`) documented household sharing as a live Premium feature — a "How does household sharing work?" Q&A, a Premium feature bullet, and a sync-troubleshooting mention — **unconditionally**, while the landing page (`page.tsx:273`) and `/upgrade` (`upgrade/page.tsx:42`) already gate every household reference behind `householdsEnabled()` (`FEATURE_HOUSEHOLDS`, defaults **off**). That made the help page the **lone surface** still advertising a dark feature — an Apple 2.3.1 / Google accurate-listing risk. Mirrored the same gate: import `householdsEnabled` from `@gm/db`, compute `householdsLive` in the server component, and drop the Q&A, the feature bullet, and the household clause of the troubleshooting note when off. `householdsEnabled()` is a pure `process.env` read (no DB) — safe in a server component / static generation, the identical gate `page.tsx`/`upgrade` use. Web typecheck + production `next build` exit 0, no missing-export warnings. This is the exact "dark-feature honesty" class runs 78/79/80 shipped (mobile paywall #594, landing/upgrade) — the help page was the last unguarded surface (runs 78-80 hadn't swept `/help`).
- **#597 — `test(reorder)`: cover the plural/singular reason copy in assessOrderReadiness [Track F].** `assessOrderReadiness` (the home-screen reorder verdict, rendered at `page.tsx:346`) interpolates two count-dependent user-facing strings — the urgent `"N items run out"` and the building `"N item(s) so far"` — but **no test asserted `reason`**, and neither the `urgentCount>1` plural branch nor the `count===1` singular building branch was exercised. Added two deterministic, mutation-sensitive tests pinning the exact copy for both branches (flipping either pluralization ternary now fails). Reviewer A `od -c` byte-verified the em-dash (U+2014) + apostrophes match the source; both inputs are reachable via `digest.topReorder` (a real top-5 list). Full `@gm/core` suite green (1041 passed).
- **#598 — `test(recipe)`: cover the no-instructions path in estimateEffort [Track F].** `estimateEffort` feeds low-energy recipe ranking and is called from `semantic-layer.ts:136` with provider recipes (TheMealDB) whose `instructions` can be null/undefined — but every existing test passed a non-empty string, leaving the `instructions ?? ""` coalescing and the false arm of `instr.trim() ? 1 : 0` uncovered. Added a test pinning the no-instructions path: a null/undefined-instruction recipe contributes **0 steps** (never a phantom step) and scores strictly below a one-sentence recipe at the same ingredient count. Both reviewers hand-computed the arithmetic (none 0.12 < oneSentence ≈ 0.153) and confirmed mutation-sensitivity (flipping the step ternary breaks the ordering). Provider `ProviderRecipe.instructions` is `string | undefined`; the `null` case still legitimately defends the `EffortInput.instructions: string | null | undefined` public type (a minor non-blocking reviewer nit noted the caller emits `undefined`, not `null`). Full `@gm/core` suite green (1040 passed).

**Rejected during scouting (verified, not shipped):**
- **`getAdminDb()` DIRECT_DATABASE_URL silent-fallback** (correctness scout) — REJECTED as **already-addressed**. `packages/db/src/client.ts:47-56` already logs a **loud, detailed `console.error`** naming the exact cause (and carries an extensive rationale comment) when `DIRECT_DATABASE_URL` is unset; the author deliberately chose log-not-throw to preserve the dev/CI single-URL fallback (the run-42 CI carve-out pattern — a real prod misconfig fails auth either way; throwing wouldn't make auth work, just marginally louder than the existing loud log, on a risky critical path). The scout mischaracterized it as "silent." Not a value-bar clear.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM; every buildable revenue lever built). Confidence statement stays UNCHECKED. Validation 8/8, 0 unmet. **Security/RLS/Track-G re-confirmed CLEAN** by the security scout (all 13 tenant tables + 3 child + 6 catalog + 2 admin tables RLS+policy through 0021; ~41 routes auth/rate-limit/zod-validated/error-hygienic; login lockout timing-safe no-enumeration; per-user LLM quota + demo per-IP/global spend ceiling; Turnstile fail-open-loud; Stripe `constructEvent` + timing-safe RevenueCat/Gmail/cron webhooks; CSP/HSTS/nosniff/Permissions-Policy; entitlements server-side; no committed secrets). A coherent converged run that closed a real store-honesty gap on a store-reviewed surface + 2 Track-F clears = success.

## 2026-07-17 (run 80) — Quiet converged run: 6-Haiku scout sweep (folded deep-audit-equivalent) + 2 file-disjoint clears (#594 mobile store-honesty, #593 Track-F coverage); 4/4 Sonnet approve first-pass; 0 abandons

Baseline green at run start (typecheck clean all 6 projects, 1036 core tests + 27 skipped, self-validation 8/8 READY 0 unmet, prod build exit 0 with no missing-export warnings, 0 open PRs, clean tree). Timing was borderline for a standalone deep audit (run 77 folded one on 2026-07-16, ~18–24h earlier; runs 78/79 both same-day treated it as not-due) — the full 6-Haiku read-only scout sweep across every deep-audit lens (security/RLS/Track-G · design/a11y/taste · living-artifact/pricing · correctness/dead-code · Track-F coverage · mobile/monetization) doubled as a folded deep-audit-equivalent. **4 of 6 lenses CLEAN (security, design, living-artifact, correctness — all NO FINDINGS).**

**Shipped (2, file-disjoint):**
- **#594 — `fix(mobile)`: don't advertise the dark Household perk on the paywall [store-acceptance honesty / monetization-parity].** The native paywall (`apps/mobile/app/upgrade.tsx`) rendered the full shared `PREMIUM_PERKS` list unconditionally via `PREMIUM_PERKS.map`, so it advertised **"Household sharing"** — a feature behind `FEATURE_HOUSEHOLDS` (off/dark pre-launch) with **no native surface**. The web upgrade page (`apps/web/app/upgrade/page.tsx:42-45`) already filters that same perk (and the Family card) via `householdsEnabled()` with an explicit "Store-acceptance honesty (Apple 2.3.1 / Google accurate-listing)" rationale — never sell a feature a new user can't reach. Mirrored the filter on mobile: `const MOBILE_PERKS = PREMIUM_PERKS.filter((perk) => perk.feature !== "household")` + a rationale comment pointing at `FEATURE_HOUSEHOLDS` (restore when a native household screen ships at launch). Mobile has no server-flag channel (`householdsEnabled()` is Node/`process.env`, server-only) and no native household screen even if the flag flipped, so a static filter is the correct dependency, not a flag round-trip. Verified `household` is the **only** 100%-dark orphaned perk — the other five (`plan_week`→plan.tsx, `discover`→discover.tsx, `remix`→remix/[id].tsx, `spend_insights`→spend.tsx, `wrapped_plus`→wrapped.tsx) each have a reachable native screen. This is the exact "dark-feature trap" the run-78/79 lessons named (run 79 rejected an always-visible mobile Family CARD on this basis; this run closes the same class in the perk BULLET list). Display-only — no entitlement/billing/purchase logic touched. `cd apps/mobile && npm ci && npm run typecheck` exit 0.
- **#593 — `test(ingestion)`: cover the no-hint branch of buildExtractionPrompt (Track F).** `buildExtractionPrompt`'s `retailerHint ? \`\nLikely retailer: …\` : ""` ternary had only its **with-hint** arm tested; the **no-hint** else-arm is real production traffic — the public demo endpoint `apps/web/app/api/public/parse-receipt/route.ts` calls `extractReceipt(client, …)` with **no** `retailerHint`, so the else-arm builds the prompt without a "Likely retailer" line (the `gmail-sync.ts` caller always supplies a truthy retailer, so the two call sites cover both arms in real code). Added a mutation-sensitive test asserting the prompt omits "Likely retailer" when no hint is given. Mutation-checked locally: forcing the ternary to always-include makes exactly this test fail (`Likely retailer: undefined.` leaks in); sibling tests still pass. Full `@gm/core` suite green (1039 passed).

**Deferred / noted (verified, not shipped this run):**
- **`gmail_import` mobile paywall surface** — both #594 reviewers noted the mobile paywall also advertises "Gmail receipt import" with no native mobile screen (no `gmail` route under `apps/mobile`; the app has no WebView wrapper). Deliberately **not** actioned this run: unlike `household` (100% dark on both platforms), `gmail_import` is a **live** feature — a premium user connects Gmail on the web and the synced pantry benefits their shared account on mobile, so the value IS deliverable; filtering it would understate real value, and building a native Gmail OAuth flow is a large/risky slice (native OAuth + a new mobile backend endpoint), not a clean file-disjoint change. Recorded here + in LOOP_MEMORY so a future run treats it as a considered judgment call (filter vs. build a deep-link-to-web connect path), not an oversight.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM; every buildable revenue lever built). Confidence statement stays UNCHECKED. Validation 8/8, 0 unmet. Security/RLS/Track-G re-confirmed CLEAN by the security scout (all tables through 0021 RLS+policy, ~47 routes auth/rate-limit/zod/error-hygienic, timing-safe webhooks, entitlements server-side, no committed secrets). A coherent converged run that closed a real store-honesty gap on the store-reviewed surface + 1 Track-F clear = success.

## 2026-07-16 (run 79) — 6-Haiku scout sweep + 3 file-disjoint clears (1 mobile-premium-parity FLAGSHIP + 2 Track-F coverage); 6/6 Sonnet approve (remix 1 review cycle: 2 real bugs fixed), 0 abandons

Deep audit not due (run 77 folded one earlier same day, <24h). Baseline green (typecheck clean all 6 projects, 1036 core tests + 27 skipped, self-validation 8/8 READY 0 unmet, prod build exit 0, 0 open PRs, clean tree). Full 6-Haiku scout sweep across disjoint lenses (Track-F coverage · security/RLS/Track-G · design/a11y/taste · living-artifact/pricing · correctness/dead-code · mobile/monetization). 4 of 6 lenses CLEAN (security, design, living-artifact, correctness all NO FINDINGS).

**Shipped (3, file-disjoint):**
- **#591 — `feat(mobile)`: AI recipe remix — close the advertised-but-missing premium perk [FLAGSHIP, monetization/premium-parity].** The mobile paywall (`upgrade.tsx:128`) advertises "recipe remix" and core `PREMIUM_PERKS` lists "AI recipe remix", but the native app had NO way to reach it — a real advertised-but-missing premium gap (buyer disappointment / refund risk) vs the web `/remix/[id]` page. Added mobile parity: (a) backend `GET /api/mobile/recipes/[id]/remix` mirroring the web `load()` — mobile-token auth + a tighter rate limit (LLM-reachable, 20/min vs the plain read's 60/min), SERVER-SIDE entitlement (`canUse("remix", isPremium(signals), billingOn)` → `upgradeRequired`), per-user daily AI quota (`quotaExceeded`), keyless-first `suggestRemix` (deterministic swap table with no key; LLM enriches when configured), degrade-by-default (any DB/LLM/upstream error → controlled JSON via `serverError`, never an uncaught 500 to a JSON client); (b) native screen `app/remix/[id].tsx` — the four axis tabs (healthier/cheaper/faster/vegan), swap cards, and real loading/error/upgrade/quota/empty states; (c) a "Remix this recipe" entry on the cook screen + a `Remix` (Ionicons magic-wand) icon in the mobile registry mirroring the web `Wand2`. Reuses already-tested core logic (`suggestRemix`/`parseAxis`/`verifyRemixDraft`, billing entitlement, rate-limit) — the route is thin glue matching the repo's mobile-route convention (no route-handler unit tests). Scope deliberately tight: the web "add swaps to list" is omitted because the mobile list endpoint is GET-only (no dead button on the screen). **Review cycle 1: Reviewer A (correctness+security) caught 2 REAL bugs — fixed and re-approved:** (1) `premium` was computed only inside `if (billingOn)`, so with `FEATURE_BILLING` off a real premium user got throttled to the free AI quota (`checkLlmQuota(userId, premium)` saw `premium=false`) — fixed by loading signals UNCONDITIONALLY (matches `plan/route.ts` + the web remix page); (2) the mobile `load()` had no staleness guard, so rapid axis-tab switching could commit an out-of-order/stale remix — fixed with a monotonic `reqGen` ref that bails after the await + guards the catch/finally setState (mirrors `cook/[id].tsx`'s `cancelled` flag). Also corrected an inaccurate "outside the try/catch" comment. Reviewer B (value/design) approved first-pass (real gap-closer, tight scope, 44px targets, a11y labels, real vector icon, no dead ends).
- **#589 — `test(recipe)`: cover the 'med' cleanup tier in estimateEffort (Track F).** `effort.test.ts` exercised the low (no-cook) and high (many-ingredient) cleanup tiers plus one-pan, but never a recipe whose `effortScore` lands in the `[0.4, 0.7)` "med" band — the middle tier the low-energy recipe ranking uses so a moderate multi-pan meal isn't treated as "easy". Added a deterministic, mutation-sensitive test (9 ingredients + 5 sentence-steps + 5 cook verbs, no one-pan → `effortScore ≈ 0.534`), asserting `onePan===false`, the band, and `cleanupLoad==="med"`. Both reviewers hand-verified the arithmetic (0.5342) and mutation-sensitivity.
- **#590 — `test(units)`: cover the dead-end null in viaItemConversions (Track F).** `units.test.ts` covered direct/reverse/2-hop item conversions plus the no-start-edge null (COUNT↔MASS with no data), but not the distinct dead-end where a start edge EXISTS yet the ≤2-hop BFS exhausts without reaching the target (the final `return null`). Added a `clove → each` case (a `clove↔g` edge present, no path to `each`) asserting null — the branch that lets `logCook`'s unit resolution degrade instead of inventing a bogus quantity. Both reviewers traced the BFS to confirm it's a genuinely distinct, previously-dark branch.

**Rejected during scouting (verified, not shipped):**
- **Mobile Family-tier paywall card** — a monetization scout proposed adding a Family ($9.99/mo) card to the mobile `upgrade.tsx`. REJECTED: the Family/household feature is behind `FEATURE_HOUSEHOLDS` (off by default), and the web upgrade page deliberately gates the Family card behind that flag ("never sell a feature a new user can't reach"). An always-visible mobile Family card would advertise a dark feature — the opposite of the store-honesty stance (same reasoning as run 78's rejected store-doc "drift").
- **`consume.ts:67` empty-token guard coverage** — a coverage scout flagged the `matches()` early-return when a tokenized ingredient name is all stop-words. Real but weak/defensive (an all-stopword ingredient name is unusual); deprioritized to avoid padding.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM; every buildable lever built). Confidence statement stays UNCHECKED. Validation 8/8, 0 unmet. Security/RLS/Track-G re-confirmed CLEAN by the security scout (all tables through 0021 RLS+policy, ~41 routes rate-limited+zod+error-hygienic, timing-safe webhooks/cron, entitlements server-side, no committed secrets). A coherent run that closed a real premium-parity gap + 2 Track-F clears = success.

## 2026-07-16 (run 78) — Quiet converged run: 5-Haiku scout sweep, 2 file-disjoint clears (#583 rankRecipes option coverage, #584 Gmail-banner tap-target a11y); 4/4 Sonnet approve first-pass; 0 abandons

Deep audit not due (run 77 folded one earlier same day, <24h). Baseline green (typecheck clean all 6 projects, 1032 core tests + 27 skipped, self-validation 8/8 READY 0 unmet, 0 open PRs, clean tree). Full 5-Haiku scout sweep across disjoint lenses (correctness/functional · security/RLS/Track-G · design/a11y/taste · Track-F coverage · living-artifact/monetization).

**Shipped (2, file-disjoint):**
- **#583 — `test(recipe)`: cover `rankRecipes` loves/dislikes, batchCook, and limit options (Track F).** `match.test.ts` covered the `lowEnergy` and `cuisineAffinity` options but four reachable sibling paths were untested: `prefs.loves` (ingredient-level boost the recommender learns from meal logging), `prefs.dislikes` (soft penalty that ranks lower without excluding), `opts.batchCook` (meal-prep up-weight, driven by the `recipes/page.tsx` mood toggle), and `opts.limit` (homepage top-N slice, passed by every page caller). Added 4 deterministic, mutation-sensitive tests. Reviewer A traced every production caller (plan/recipes/discover/use-it-up pages, semantic-layer, all three mobile API routes) confirming the options are real public API, not impossible cases; sibling-precedent to the tested `cuisineAffinity`. +4 tests → 1036 core passing.
- **#584 — `fix(a11y)`: enlarge Gmail-banner dismiss button to a 44px tap target (design bar).** The dismiss control on `gmail-upgrade-banner.tsx` used `p-1` + a 16px icon (~24px hit area) — below the 44px minimum the rest of the app holds (theme-toggle, the `h-11 w-11` icon-tile utility in globals.css). Switched to the repo's `flex h-11 w-11 items-center justify-center` idiom: full 44px target, visually quiet (no chrome until hover), no layout regression (`items-start` row, no cross-axis stretch). Banner renders on home + high-traffic pages, so it's a real WCAG 2.5.x gap, not a buried surface. Typecheck + prod build green.

**Rejected during scouting (verified, not shipped):**
- **Receipt-verify refund asymmetry (`ingestion/verify.ts:46`)** — a correctness scout flagged that `sum - totalCents > tolerance` only catches over-extraction for positive totals, so a negative-total refund with over-summed lines would pass. FALSE POSITIVE: lines 35–37 already reject any negative `lineTotalCents`, so by the reconciliation every line total is ≥0 and the negative-sum scenario is unreachable. No change.
- **Family-tier / household-sharing store-doc "drift"** — a living-artifact scout flagged that the `premium_family` tier + household feature (canonical in the billing catalog, BUSINESS_CASE, privacy-disclosures) are absent from the store ASO docs. NOT a bug: `upgrade/page.tsx:38–45` deliberately omits the Family card + household perk until `FEATURE_HOUSEHOLDS` is flipped at launch ("never sell a feature a new user can't reach"), so the store listings are consistent with the pre-launch dark state. Adding Family to store copy would risk advertising a dark feature — the opposite of the intended fix. Left as-is.
- **`log-cook.ts:129` unit-conversion coverage** — real but DB-bound (`db.select().innerJoin`); this repo keeps core tests DB-free, so it would need heavy mocking the repo avoids. Not a clean clear.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM; every buildable lever built). Confidence statement stays UNCHECKED. Validation 8/8, 0 unmet. Security/RLS/Track-G re-confirmed CLEAN by the security scout (all tables through 0021 RLS+policy, ~41 routes rate-limited+zod+error-hygienic, timing-safe webhooks/cron, entitlements server-side, no committed secrets). A coherent converged run with 2 real clears = success.
...
## Run 86 — 2026-07-19 — DEEP AUDIT (6-Haiku lens sweep) + 3 file-disjoint clears; 6/6 Sonnet approve; a real Track-G G7 wallet-drain gap closed on BOTH platforms
DEEP AUDIT due since run 83 (~24h). Baseline gate green at start (typecheck 6/6, 1043 core tests + 27 skipped, production build exit 0 no missing-export, self-validation 8/8 READY 0 unmet, 0 open PRs, clean tree). Six read-only Haiku lenses over the whole repo (security/RLS/Track-G · correctness/functional/uncaught-throws · Track-F coverage · design/a11y/taste · artifact-freshness/monetization · perf/mobile-parity) → 1 real security finding (both halves fixed) + 1 Track-F clear; every other candidate rejected/deferred with evidence. All 3 changes file-DISJOINT; each got 2 Sonnet reviews (A correctness/security + B value) — all 6 APPROVE (one after a re-judge on a corrected premise, below); 0 abandons.

### PR #618 + #620 — harden(cook): gate the "I cooked this" macro LLM behind the per-user daily quota, on BOTH platforms (Track-G G7) [FLAGSHIP, security]
Cook-logging estimated meal macros via a `GeminiClient` with **no `checkLlmQuota` gate** on either surface — `apps/web/app/api/mobile/cook/route.ts` (mobile, PR #618) had only a `cook-log-write` 30/min rate-limit (~43,200/day, unrelated to the daily cap), and `apps/web/app/lib/cook-actions.ts` `logCookedRecipe` (web server action, PR #620) had **no rate limit at all**. Every other LLM surface (ask, scan, make, import, onboarding, mobile plan/remix, web remix) already charges the G7 per-user daily spend ceiling; cook was the lone holdout on both platforms. An authed user could call either repeatedly with FDC-unresolvable ingredients and drain the paid Gemini API past the 10-free/100-premium daily cap. Fix (both): load preference signals → `isPremium` sets the tier → `checkLlmQuota` **before** constructing the LLM (mirrors remix/plan); on exhaustion, degrade to keyless FDC-only macros rather than blocking the core pantry-drawdown write (`estimateMealMacros` makes exactly one Gemini call, so charging one slot is accurate). Shipped file-disjoint so each auto-merged independently. Process note: Reviewer A on #620 first returned REQUEST_CHANGES on a **stale premise** — reviewing the web branch (cut from main) it saw main's still-unfixed mobile route and concluded the "both platforms" claim was false; it revised to APPROVE once shown #618 fixes that exact file the same run. Resolved by a factual correction (maker≠checker), not an override; the web PR body cross-references #618 so a triager needn't rediscover it.

### PR #619 — test(units): cover convert()'s unknown-code + zero-factor-edge null paths [Track-F]
`UnitConverter.convert` receives unit codes parsed from arbitrary provider/user recipe text (`parseMeasure` extracts a raw token with no validation). Two reachable defensive branches on the pantry-drawdown path were uncovered: an unknown code (`index.ts:69` `if (!from || !to) return null` → logCook records the ingredient unmeasured, else `undefined.dimension` throws) and a zero-factor item conversion (`:109` `if (c.factor === 0) continue` — meaningless, and `1/0=Infinity` on the reverse edge — dropped → no path → null). Added two focused regression tests; `src/units` branch coverage 89.74% → 95.12%. Lines 124/141 left uncovered on purpose (a bidirectional-graph dead guard + a direction-symmetric water-density mirror — testing them is impossible-case padding).

### Rejected (verified false / churn, with evidence)
- **loadEnv/DB reads outside try in the gmail/stripe/revenuecat webhooks + gmail/digest/lifecycle crons**: the already-examined class (runs 57/74/77). A `loadEnv()` throw = whole-app misconfig = correct fail-loud; a cron 500 → the next scheduled run reconciles (cursor-based/idempotent, no data loss); the per-item webhook sync is already try-wrapped; the failure mode is a benign Vercel retry with no real-outcome change. Guarding them changes nothing.
- **Sequential ledger-append / trigram N+1 in `vision/persist.ts` (scan confirmations) + `capture/add.ts` (quick-add)**: the ingest N+1 class prior audits (runs 38/41) held sub-bar — LLM-bound, correctness-sensitive (the append-only ledger reprojection is per-(user,item) by design), and low-frequency (a user confirms/adds a handful of items). Not a bug.
- **`manage-subscription` `←` "Back to profile" entity → icon**: within the established inline-text-arrow convention prior audits explicitly declined to flag (run-59/72 lesson; the registry rule targets standalone glyphs like ▾/★, not inline affordance arrows).
- **`wrapped` shareText emoji**: idiomatic social-share copy, not a UI icon — the "never emoji" rule targets UI icons; removing them would weaken the share surface.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K at median = owner-GTM; the honest ~$67K gap is a ~2.7–3× download multiplier, not a buildable lever — every pricing/tier/conversion/retention/margin lever is built). Confidence statement stays UNCHECKED. Validation 8/8, 0 unmet; ship gate MET (A). A coherent converged run: a real security clear on both platforms + a Track-F clear + a full 6-lens deep audit, 6/6 approvals, 0 abandons.

## Run 87 — 2026-07-19 — targeted independent re-verification (3-lens adversarial) + 1 file-disjoint Track-G clear; 2/2 Sonnet approve; 0 abandons
Fresh run whose tree moved only by the run-86 merges since the same-day deep audit → DEEP AUDIT not due (<24h). Per the run-85 lesson, the disciplined maker≠checker move on the CURRENT commit is a targeted INDEPENDENT re-verification (a few lean adversarial scouts on the highest-yield lenses + own spot-checks), NOT trusting the prior run's word and NOT a redundant full fan-out. Own maker spot-checks first: migration chain 0021 intact; BUSINESS_CASE SUMMARY valid YAML (base $33,450 == body, floor_met false); prices 499/3999/999/7999¢ byte-consistent (billing ↔ growth snapshot ↔ business case); validation 8/8 READY 0 unmet; QUALITY_SCORECARD overall A, ship gate MET. Then 3 Haiku adversarial scouts (security/RLS/Track-G · monetization non-reach lever · correctness/artifact/design), each told the run-86 deferred list so they hunt only genuinely-NEW value. Monetization scout: **NO BUILDABLE NON-REACH LEVER** (the $67K gap is entirely reach/owner-GTM — every ARPU/tier/conversion/retention/margin lever is built; independently corroborates runs 82–86). Correctness/artifact/design scout: **NO NEW FINDINGS**. Security scout: **1 genuinely-new real finding** (below), which I independently confirmed before acting.

### PR #622 — fix(auth): constant-time credential verify to close username enumeration (Track-G) [FLAGSHIP, security]
All three login entry points — `apps/web/auth.ts` (primary web NextAuth `authorize`), `apps/web/app/api/v1/auth/token/route.ts`, `apps/web/app/api/mobile/auth/route.ts` — shared a `!user?.passwordHash || verifyPassword(...)` short-circuit that **skips the `scryptSync` KDF entirely when the username doesn't exist** (or is a Google-only account with no password hash). Because scrypt is deliberately ~tens of ms, the "no such user" path is measurably faster than a real-but-wrong password → a **timing oracle** an attacker enumerates valid usernames with. A per-IP rate-limit (defeated by distributed IPs) and a generic `"invalid credentials"` message do NOT close a timing side-channel — the two latency classes stay distinguishable. Fix: new `verifyPasswordConstantTime(plain, stored|null)` in `@gm/core/crypto` runs the full scrypt cost against a **lazily-derived throwaway hash** on the no-hash path (never matches a real password; the discarded result can't cause a bypass — it unconditionally returns false), so "unknown username" is timing-indistinguishable from "wrong password". All three sites switched to verify FIRST (unconditionally) then branch — no remaining short-circuit; the `|| !user` after the call is load-bearing for TS narrowing of the success branch (cleaner than a `user!` assertion), not dead. Lockout/`recordFail`/`clearLock` semantics and all 401/503/400 responses unchanged. 4 unit tests (correct→true, wrong→false, absent(null/undefined/"")→false-not-throw, malformed→false). Reused the existing auth capability — no new capabilities.json entry. Both Sonnet reviewers APPROVE first-pass; both raised the same non-blocking note (the first no-user request on a cold container pays a one-time extra scrypt to compute the dummy hash — a one-time-per-container artifact swamped by cold-start jitter, not a per-request oracle; eager top-level init would fix it but tax every cold `@gm/core/crypto` import, so the lazy tradeoff is deliberate).

### Rejected / no-op (with evidence)
- **A eager (top-level) dummy-hash init** to close the cold-container one-time edge: deliberately NOT done — it would add ~80ms scrypt to every cold import of `@gm/core/crypto` (including many tests and any code path importing crypto) to close a non-oracle one-time-per-container artifact. Lazy memoization is the correct tradeoff (both reviewers concurred it's non-blocking).
- **Owner/CI-gated open issues** (#571 evals cron → weekly, #320 perf-budget CI gate): both require editing `.github/` (DO-NOT-TOUCH — a headless run can't answer the sensitive-file prompt). Left for the owner. #442/#356 media-gen, #413/#363 computer-use validator, #401 cold-email: owner-infra/key-gated, not buildable keyless this run.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K at median = owner-GTM; no buildable non-reach lever, independently re-confirmed this run). Confidence statement stays UNCHECKED. Validation 8/8, 0 unmet; ship gate MET (A). A quiet, coherent converged run: one real Track-G security clear + independent 3-lens adversarial re-verification of convergence on the current commit, 2/2 approvals, 0 abandons. Padding to a count is the worst failure mode — a quiet convergent run is a SUCCESS.

## Run 88 — 2026-07-19 — 5-Haiku scout sweep (folded deep-audit-equivalent) + 4 file-disjoint clears; 8/8 Sonnet reviews approve (1 review cycle on #628); 0 abandons
Baseline green at start (typecheck 6/6, 1049 core tests +27 skipped, prod build exit 0 no missing-export, self-validation 8/8 READY 0 unmet, 0 open PRs, clean tree). DEEP AUDIT not due (run 86 ran a standalone 6-lens sweep the same day, <24h). Five Haiku scouts (Track-F coverage · security/RLS/Track-G · design/a11y+taste · mobile-parity/living-artifacts · correctness/dead-code): SECURITY and CORRECTNESS lenses returned NO findings (both re-confirmed the converged posture — security: all tables through 0021 RLS+policy, ~41 routes hardened, timing-safe webhooks, entitlements server-side, no committed secrets; correctness: depletion/reorder/nutrition/recipe/verify/spend math traced clean). 4 file-disjoint clears selected; each got 2 Sonnet reviews (A correctness/security + B value/design).

### PR #625 — docs(marketing): drop dark household-sharing claims from staged email/press/content copy [living-artifact / store-honesty]
Household sharing is gated behind `FEATURE_HOUSEHOLDS` (default off) and unreachable on web + mobile. The web/mobile paywalls and the App Store / Play store metadata already OMIT it while dark (run-78 established that advertising it is an Apple 2.3.1 / FTC risk), but three STAGED launch artifacts still advertised it as an available Premium perk: `docs/brand/EMAIL_LIFECYCLE.md` (T1 trial-welcome bullet list, A3 spotlight feature block, T2/T3 expiry "features pause" lines), `docs/brand/PRESS_KIT.md` ("who it's for" + press-release subscription blurb), `docs/brand/CONTENT_DRAFTS.md` (launch + upgrade email drafts). Replaced each mention with a real, reachable Premium feature (AI recipe remix / unlimited Discover / advanced spend insights — all in `packages/core/src/billing` `PREMIUM_PERKS`, matching the store metadata feature set), so the premium value story stays intact and consistent. Prices untouched. Both Sonnet reviewers APPROVE (A: replacements are real live perks matching `PREMIUM_PERKS` + store metadata, all 7 mentions caught, clean prose, no scope creep; B: genuine drift vs the shipped gated state, mirrors the run-78/79 precedent, no invented claims, no price drift). One non-blocking prose nit (triple "unlimited" on PRESS_KIT) polished before merge.

### PR #626 — test(units): cover the mass→volume water-density heuristic direction [Track-F]
`UnitConverter.viaWaterDensity` handles both cross-dimension directions under the ~1 g/ml assumption, but only VOLUME→MASS was tested — the `MASS && VOLUME` clause on `units/index.ts:141` was an uncovered branch. Added a mirror test (500 g → 500 ml; 2 kg → 2 l) asserting qty, `heuristic` method, and low confidence → branch coverage 95.12%→97.36%. Mutation-sensitive (dropping the MASS→VOLUME clause makes `convert()` return null → the test throws). Reviewer B caught that the branch's real UNFILTERED caller is `ingest.ts` (a receipt line in a mass unit vs a volumetric canonical base) — `logCook` also reaches it but DISCARDS `method==="heuristic"` results — so the test comment was corrected to cite ingestion. Both APPROVE.

### PR #627 — fix(a11y): raise pantry expired-resolution chips to the 44px touch target [design/a11y]
The "Still good?" resolution buttons (Still have it / Used it / Tossed it) on likely-expired pantry items were `btn-ghost btn-sm` (~28px), below the app's 44px standard — even though the Remove control in the SAME row already meets it. These chips record a real append-only ledger event. Added `min-h-[44px]`. Both APPROVE (2/2, merged before the #628 review feedback below landed).

### PR #628 — fix(a11y): raise the review-queue bulk actions to the 44px touch target [design/a11y]
The receipt-review toolbar's bulk "Add to pantry" (writes selected items into the pantry ledger) + dismiss controls were `btn-sm` (~28px). **1 review cycle:** Reviewer B REQUEST_CHANGES on v1 (`btn-primary/ghost btn-sm min-h-[44px]`) — gluing `min-h-[44px]` onto `btn-sm` (`px-3 py-1.5 text-xs`) yields a tall pill wrapping a 12px label, off-precedent vs the app's 44px controls (`.chip-tap`, cook-mode, onboarding) which scale padding+font WITH height. Revised to base `.btn` + `min-h-[44px]` (dropping `btn-sm`), the exact `onboarding-flow.tsx` CTA precedent → both reviewers APPROVE on v2.

### Rejected / deferred (with evidence)
- **Track-F `expiring.ts:51` domain-null branch** — REJECTED: file is already at 100% (the `{now}`-only call covers the no-domain path; the household soap row covers the filter). Always check a coverage claim against the report before selecting.
- **Track-F `log-cook.ts:129` heuristic guard** — REJECTED: DB-bound (behind `db.select().innerJoin`), skipped per the run-78 DB-free-core-tests lesson.
- **Track-F `user-model.ts:113` `default:break`** — REJECTED: impossible-case guard (an unknown topic kind is unreachable through the typed union).
- **Design homepage nav `btn-sm`** (`page.tsx:299/308/315 Profile/Sign out/Sign in`) — REJECTED: an app-wide compact-header idiom; changing only home breaks header consistency, and text header links at ~28px are a deliberate pattern (nav mis-taps are cheap, unlike ledger-writing controls).
- **Design `staples/page.tsx:169` "Set"** — DEFERRED (not rejected): a real sub-44px write control, but a correct fix needs the adjacent ~28px number input raised too for balance + a browser visual check → future `.chip-tap` polish pass. Recorded with the pantry-#627 chips as a design-polish follow-up.

**Design-idiom follow-up (recorded, not blocking):** #627 shipped the `btn-sm + min-h-[44px]` pattern (2/2-approved + auto-merged before #628's reviewer flagged it); it's a11y-compliant + functional so it was left as-is rather than re-touching just-merged code. The pantry resolution chips + `staples/page.tsx:169` "Set" (+ its number input) should adopt the proportionate `.chip-tap` idiom (`min-h-[44px] px-4 text-[0.95rem]`) in a future pass with a real browser visual check.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K at median = owner-GTM; no buildable non-reach lever). Confidence statement stays UNCHECKED. Validation 8/8, 0 unmet; ship gate MET (A). SECURITY/RLS/Track-G + CORRECTNESS re-confirmed CLEAN. Four coherent file-disjoint clears (1 store-honesty living-artifact, 1 Track-F, 2 a11y) — a productive convergent run.

## Run 89 — 2026-07-20 — DEEP AUDIT (4-Haiku lens sweep, due since run 86 ~24h) + 2 file-disjoint a11y clears with the corrected 44px idiom; 4/4 Sonnet reviews approve (1 review cycle on #630); 0 abandons
Baseline green at start (typecheck 6/6, `@gm/core` test suite exit 0 [~917 cases], validation 8/8 READY 0 unmet, migration chain 0021 intact + wired in order, prices 499/3999/999/7999¢ byte-consistent across billing↔BUSINESS_CASE↔store↔privacy, SUMMARY valid YAML base $33,450/floor_met false, quality ship gate MET (A), 0 open PRs, clean tree). DEEP AUDIT was DUE — run 86 was the last STANDALONE deep audit (~24h/4 runs prior; runs 87/88 were targeted/folded). Four Haiku deep-audit scouts, each a distinct lens over the whole repo; all findings verified by the maker before selecting.

- **SECURITY & ABUSE (Track G): NO NEW FINDINGS.** All tables through 0021 have RLS+policy; ~41 routes rate-limited + zod-validated + error-hygienic; constant-time auth verify (#622) + login lockout; per-user LLM quota + demo per-IP/global ceiling; captcha on public forms; CSP/HSTS/nosniff; Stripe `constructEvent` + timing-safe RevenueCat/Gmail/cron webhooks; entitlements server-side; no committed secrets.
- **CORRECTNESS & DEAD CODE: 1 candidate → REJECTED (false positive).** The public demo `parse-receipt` route returns HTTP 200 with `{error}` (no `items`) when extraction succeeds but finds zero line items. The scout claimed a client crash, but the SOLE consumer (`apps/web/app/demo/demo-client.tsx:105`) guards on `if (!res.ok || !data.items)` → it handles the no-items case correctly (renders the calm error, no crash). 200 for "parse succeeded, zero items" is a defensible distinction from the 502 server-error path; changing to 422 has ZERO behavior change on the only caller → does not clear the value bar.
- **DESIGN/a11y: multiple real sub-44px `btn-sm` interactive controls found** → 2 coherent surfaces shipped (below); staples deferred.
- **ARTIFACT-FRESHNESS & MONETIZATION: pricing byte-consistent (no drift); NO buildable non-reach revenue lever.** Every pricing/tier/conversion/retention/margin lever is built (referral loop, month-3 annual nudge, win-back, Family tier dark behind `FEATURE_HOUSEHOLDS`); the ~$67K floor gap is a ~2.7–3× download multiplier = owner-GTM (#190). Reach-gated convergence independently re-confirmed.

### PR #630 — fix(a11y): raise the core cook-loop action buttons to the 44px touch target [design/a11y]
The "Cook →" links (recipes + use-it-up) and the shared one-tap "Cooked it" button (records a real cook-log + pantry-drawdown ledger event) were `btn-sm` (~28px). **1 review cycle:** v1 shipped `btn-secondary/primary btn-sm min-h-[44px]` — Reviewer B REQUEST_CHANGES, correctly citing the run-88 `LOOP_MEMORY` design-idiom lesson: `btn-sm + min-h-[44px]` is the documented ANTI-PATTERN (a 44px box wrapping a 12px `text-xs` label = tall pill). Reworked to base `.btn` sizing (`px-4 py-2.5 text-sm`) + `min-h-[44px]`, dropping `btn-sm` — the corrected #628 idiom → both reviewers APPROVE on v2. `CookedItButton`'s shared default className carries the fix to both call sites (verified no third caller / no caller overrides the default). Merged.

### PR #631 — fix(a11y): raise first-run & conversion CTAs to the 44px touch target [design/a11y]
Three sub-44px `btn-sm` CTAs: the Getting Started checklist action links (first-run activation), the PWA "Install" prompt button, and the Gmail-import "See Premium" conversion CTA. Applied the CORRECT idiom directly (base `.btn` + `min-h-[44px]`, dropping `btn-sm`). The install-prompt dismiss "X" already meets 44px (`h-11 w-11`) — untouched. JSX comments placed as valid element/expression siblings inside their parent (typecheck confirmed the initial in-attribute/in-`&&` placements were syntax errors and fixed them). First-pass 2/2 approve.

### Rejected / deferred (with evidence)
- **parse-receipt 200-with-error** (correctness) — REJECTED false positive (sole client guards on `!data.items`; 200-for-zero-items is a defensible distinction from 502; zero behavior change).
- **`staples/page.tsx` "Set" + autopilot toggle** (design) — DEFERRED (as in run 88): the "Set" button sits in a `flex items-center gap-1.5` next to a ~28px number input; a correct fix needs the input raised too for visual balance + a real browser visual check, which this environment cannot reliably perform. Building it blind risks a visual mismatch → left for a design-polish pass that can do the browser check. The autopilot toggle alone is a clean drop, but splitting the surface would be incoherent → whole surface deferred.
- **webhook/cron loadEnv + ingest N+1** — not re-flagged this run; the already-examined benign classes (runs 38/41/57/74/77) stand.

**PROCESS LESSON — read the latest LOOP_MEMORY design-idiom lesson BEFORE choosing an idiom; the chronologically-latest merged commit can BE the anti-pattern the same run's lesson corrects.** I initially modeled the cook-loop fix on #627 (which shipped `btn-sm + min-h-[44px]` and auto-merged BEFORE the run-88 feedback landed) rather than the run-88 lesson + the corrected #628. Reviewer B's REQUEST_CHANGES was a CORRECT premise (not a stale one) — resolved by ADOPTING the prescribed fix, not overriding. When two exemplars conflict, prefer the one the LESSON endorses (#628 base `.btn`), not the latest-merged (#627). Maker≠checker earned its keep: the Sonnet reviewer caught a real design-idiom regression the maker's #627-anchored taste missed. This closes the run-88 follow-up for the cook-loop + first-run/conversion surfaces with the right idiom; only the staples surface (needing a browser check) remains.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K at median = owner-GTM; no buildable non-reach lever, independently re-confirmed this run). Confidence statement stays UNCHECKED. Validation 8/8, 0 unmet; ship gate MET (A). DEEP AUDIT: security + correctness + monetization all re-confirmed clean/converged; the only real findings were a11y touch-targets, 2 shipped with the corrected idiom. A coherent convergent run.

## Run 90 — 2026-07-20 — targeted independent re-verification (4-Haiku adversarial) + 1 file-disjoint correctness/UX-honesty clear (ingest count); 3 Sonnet reviews approve (1st cycle both APPROVE, 2nd cycle a folded verb-agreement delta APPROVE); 0 abandons

DEEP AUDIT NOT due (run 89 ran a standalone 4-lens sweep the same day, 2026-07-20; the tree moved only by the run-88/89 merges since). Per the run-85/87 lesson, the disciplined maker≠checker move on a fresh run since a same-day deep audit is a targeted INDEPENDENT re-verification of convergence on the CURRENT commit — not trusting the prior run's word, not a redundant full 6-lens fan-out. Baseline green at start (typecheck 6/6, 1050 core tests pass + 27 skipped, prod build exit 0 no missing-export, validation 8/8 READY 0 unmet, migration 0021 intact, prices 499/3999/999/7999¢ byte-consistent billing↔BUSINESS_CASE, SUMMARY valid YAML base $33,450 == body base scenario / floor_met false, 0 open PRs, clean tree). Also evaluated the sole genuinely-unbuilt buildable-looking ROADMAP box (§29 deployed-app validator): the app IS live (grocery-manager-web.vercel.app HTTP 200) and Browserbase keys are present, BUT producing §29's REAL outcome (a green sweep of the DEPLOYED app with genuine flow counts) requires driving past the live site-gate (SITE_GATE_SECRET absent here) + prod test-account/Stripe-test provisioning this env can't supply — so building the harness would ship a spec that can't be run (a fabricated green the DoD forbids). §29 is effectively owner-gated like §11/§34, matching why 8 prior runs deferred it.

Four lean adversarial Haiku scouts on the highest-yield lenses: security/Track-G → **CLEAN** (no new findings; all tables through 0021 RLS+policy, ~41 routes rate-limit+zod+error-hygienic, constant-time auth verify, per-user LLM quota on every LLM surface incl. add-receipt, timing-safe webhooks, CSP/HSTS/nosniff, no committed secrets); monetization non-reach lever → **NO BUILDABLE NON-REACH LEVER** (independently re-confirmed the $67K floor gap is entirely reach = a ~2.7–3× download multiplier = owner-GTM #190; every pricing/tier/conversion/retention/expansion/margin lever already built); design/a11y/artifact → **CLEAN** (design system coherent, ≥44px targets, humanize/titleCase, BUSINESS_CASE/README/VISION consistent with code); correctness/functional-reality → **1 genuinely-new real finding**, which I independently confirmed before acting.

### PR (ingest count honesty) — fix(ingest): report items actually added to the pantry, not total lines parsed [correctness / UX-honesty / core-loop]
The receipt-ingest success copy led with `linesIngested` — "Added N items from your receipt." — but `linesIngested` counts EVERY parsed line, including the low-confidence ones parked in the Review inbox that are NOT in the pantry yet. A user who snapped a 10-item receipt with 4 unresolved lines was told "Added 10 items" and then found only 6 in their pantry (violates CLAUDE.md's "render real DB-derived values only"). Fixed at the SOURCE, not the display layer: added an `addedToPantry` field to `IngestResult`, incremented at EXACTLY the `appendLedgerAndReproject` call site (so it's ledger-truth, not the derived arithmetic `linesIngested − needsReview` duplicated in two views that could silently drift if a future line-item path breaks the `!needs ⟺ ledger-appended` coincidence). Threaded through the Gmail rollup (`GmailSyncSummary`/`accumulateParseResult`/`emptyGmailSyncSummary`) — flows automatically into `ParseReceiptResult` (= `Awaited<ReturnType<typeof ingestReceipt>>`), matching the existing granular-count pattern. Both ingest surfaces (snap-a-receipt + Gmail "Sync now") now lead with items actually added to the pantry, reporting review parkers separately; the all-parked edge (`addedToPantry === 0, linesIngested > 0`) degrades to "Captured your receipt." rather than "Added 0 items", and the pantry banner keeps a `sp.added ?? sp.lines` fallback for in-flight redirect links minted before this deploy. Keyless regression guard: the pure `gmail-sync.test.ts` rollup assertions (addedToPantry 3 for ingested(5,2); 4 for the mixed batch) + a DB-level invariant assertion in the skip-gated integration test (`addedToPantry === linesIngested − needsReview`). Reviewer A (correctness+security) APPROVE: the invariant `linesIngested === addedToPantry + needsReview` is STRUCTURALLY guaranteed by the loop (each iteration increments exactly one of addedToPantry/needsReview; lines++ unconditionally), not just true for the fixtures; both return paths covered; no tenant/secret concern. Reviewer B (value-first) APPROVE: real core-loop trust fix, source-of-truth field justified over display arithmetic, copy calm/honest across singular/plural/zero-added. Reviewer B flagged a PRE-EXISTING verb-agreement bug in the SAME review-note copy — `need${plural(n)}` misapplies the NOUN pluralizer to the VERB "need", inverting agreement in BOTH directions ("1 need" should be "1 needs"; "2 needs" should be "2 need"). Since I was already rewriting those exact strings, I folded the fix in (`need${n === 1 ? "s" : ""}`) as a coherent completion of the copy fix and ran a focused 2nd review cycle → APPROVE (grammar correct at both sites, noun `plural` usage intact, no regression).

### Rejected / deferred (with evidence)
- **§29 deployed-app validator** (ROADMAP box) — DEFERRED (owner-gated in practice): live app + Browserbase keys present, but the REAL green-sweep outcome needs the live site-gate secret + prod test provisioning this env lacks; a harness that can't be run = a spec/fabricated-green the DoD forbids. Same boundary as §11/§34.
- **mobile capture "Added N to your list"** (correctness) — NOT the same bug: a separate natural-language quick-add → shopping-list flow with its own correct `added` count; out of scope.
- **webhook/cron loadEnv + ingest N+1** — not re-flagged; the already-examined benign classes (runs 38/41/57/74/77/86) stand.

**LESSON — a "count" surfaced in UI copy must be sourced from the SAME operation the verb claims.** "Added N" that reads from a PARSE count (`linesIngested`) rather than the WRITE count (ledger appends) is a fake-data bug even though every number is real — the number is real but it's the wrong number for the verb. The robust fix is a source-of-truth field incremented at the actual write site, not arithmetic re-derived at each view (which drifts when the "resolved ⟺ written" coincidence breaks). When you touch a piece of user-facing copy, audit the WHOLE fragment for adjacent correctness bugs (here a verb-agreement inversion from a noun-pluralizer misused on a verb) — fixing them in the same coherent copy change is completion, not scope creep.

**PROCESS LESSON — folding a reviewer-surfaced adjacent nit into an already-approved diff is legitimate WHEN it's in copy you already own, but it demands a fresh review pass.** Both reviewers approved the substantive change; Reviewer B's non-blocking verb-agreement observation was correct AND under-stated (wrong in the plural case too). Rather than defer a known-wrong grammar bug in copy I was shipping, I folded it in and re-reviewed the delta — respecting maker≠checker (the amended diff is not the approved diff) without a full re-audit.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K at median = owner-GTM; no buildable non-reach lever, independently re-confirmed this run). Confidence statement stays UNCHECKED. Validation 8/8, 0 unmet (the ingest fix reuses existing capabilities — no new capability to register); ship gate MET (A). A quiet, coherent convergent run — one real correctness/UX-honesty clear on the core loop; padding to a count is the worst failure mode, a quiet convergent run is a SUCCESS.

## Run 91 — 2026-07-22 — targeted independent re-verification (4-Haiku adversarial) + 1 file-disjoint nutrition correctness clear (#634) + 1 LAUNCH handoff fix

DEEP AUDIT NOT due (run 89 ran a standalone 4-lens sweep ~2 runs prior, < the ~4-run/~24h cadence; the tree moved only by the run-90 #633 merge since). Per the run-85/87/90 discipline, the maker≠checker move on a fresh run since a recent deep audit is a targeted INDEPENDENT re-verification on the CURRENT commit — lean adversarial Haiku scouts on the highest-yield lenses + own maker spot-checks — not trusting the prior run, not a redundant full fan-out.

Own maker spot-checks: typecheck 6/6, 1050 core tests pass +27 skipped, prod build exit 0 no missing-export, validation 8/8 READY 0 unmet, 0 open PRs, prices 499/3999/999/7999¢ byte-consistent, BUSINESS_CASE_SUMMARY valid YAML (base $33,450 == body, floor_met false).

Four lean adversarial Haiku scouts: **security/Track-G** → 1 OWNER-GATED finding (see below), all other lenses CLEAN; **monetization** → NO buildable non-reach lever (reach-gated #190 re-confirmed for the ~9th consecutive run); **correctness/core-loop** → 1 genuinely-new real finding (shipped as #634); **design/a11y/artifact** → 2 candidates rejected.

### PR #634 — fix(nutrition): scale meal macros by fractional servings to match the pantry drawdown [correctness]
`estimateMealMacros` opened with `const s = Math.max(1, servings)`, over-clamping any `0<servings<1` up to a full 1×. But `logCook`→`planConsumption` scale the pantry drawdown by the SAME fractional `servingsScale` (`x>0?x:1`, preserved). So a half-batch cook (servings 0.5) drew down 0.5× of the pantry but logged FULL-recipe macros — 2× what was eaten — contradicting the function's own docstring ("the stored figure is for everything actually cooked"). Fixed to `Number.isFinite(servings) && servings > 0 ? servings : 1`, aligning the third scaling site with the other two; also closes a latent `Math.max(1, NaN) = NaN` propagation bug (Reviewer A). +2 regression tests (fractional 0.5 ⇒ half macros; 0/-2/NaN ⇒ 1×). Gate green: 1052 core tests, typecheck 6/6, prod build exit 0. 3 Sonnet reviews: Reviewer A APPROVE 1st pass; Reviewer B REQUEST_CHANGES on framing → APPROVE cycle 2.

### LAUNCH.md handoff-completeness fix (this bookkeeping PR)
The security scout's top finding was in PENDING_OPS but ABSENT from LAUNCH.md's owner remaining-steps checklist, which also had NO provider-spend-cap step at all — despite "PROVIDER SPEND CAPS + alerts" being a required Human-Core handoff item. Added LAUNCH.md **Step 10** (provision an Upstash-Redis shared rate-limit/quota/demo-spend store for multi-instance safety + set Gemini/Upstash/Stripe spend caps + alerts before driving public traffic; renumbered Submit/GTM/Post-launch → 11/12/13). Also filled a run-90 gap in LOOP_HEALTH.changes_shipped (the chain jumped 89→88, missing #633).

### Rejected / deferred (with evidence)
- **Multi-instance in-memory rate-limit/quota/demo-spend ceiling** (security/Track-G) — REAL at scale but OWNER-GATED, correctly DEFERRED (already tracked in PENDING_OPS `llm-quota-redis-upgrade`). The counters live in per-process Maps, so a scaled-out Vercel deployment's effective cap = cap×instances. The real fix needs owner Upstash provisioning; a Redis-backed wrapper is UN-PROVABLE keyless in CI (the degrade path stays in-memory), so building it would ship an un-exercised critical protection path — the DoD trap. Now SURFACED into the owner checklist (LAUNCH Step 10, above) so it's no longer PENDING_OPS-only.
- **wrapped shareText emoji** (design) — REJECTED: `🧺/🍳/💸` in `wrapped/page.tsx` are in plain-text SOCIAL-SHARE copy (posted to Twitter/Messages), not UI icons — the "never emoji" rule targets in-interface icons (lucide registry), and SVG icons are impossible in a share string. Removing them would be net-negative + churn.
- **bottom-nav 36px icon-span** (a11y) — REJECTED: the `h-9` span is inside the full `<a>` tap target (~89×61px, well over 44px in both dimensions); the whole link is clickable, not just the icon.

**LESSON — a fractional "scale" must be applied identically everywhere it flows.** When one function clamps `servings` to ≥1 while its sibling scaling functions preserve the fraction, a fractional input silently desyncs the two outputs (macros vs pantry drawdown). Fix at the shared boundary with the SAME `x>0?x:1` guard the siblings use, not a stricter clamp — and grep the other scaling sites to confirm the convention before choosing the guard.

**PROCESS LESSON — verify a scout's reachability claim per-surface before writing the value framing.** The scout (and my first summary) said the bug hit "all 3 cook surfaces"; Reviewer B traced it and only the mobile API route + a crafted web-action POST actually reach fractional servings (the web form `min=1`/`step=1` and the mobile stepper are integer-clamped by construction). The fix was correct but the CLAIM was inflated. An overstated reachability claim pollutes the audit trail even when the code is right — VERIFY each surface's input control + server-side validation before framing. Resolved by correcting the commit/PR framing and re-running a 2nd review cycle to convert Reviewer B → APPROVE (maker≠checker satisfied via honest re-framing, never an override).

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM; no buildable non-reach lever, independently re-confirmed). Confidence statement stays UNCHECKED. validation 8/8, 0 unmet; ship gate MET (A). A quiet, coherent convergent run — one real nutrition-loop correctness clear + one owner-facing security/cost handoff fix; a quiet convergent run is a SUCCESS.

## Run 92 — 2026-07-23 — DEEP AUDIT (7-Haiku lens sweep, DUE since run 89 ~4 days/>24h) + 4 file-disjoint clears (2 mobile correctness bugs #637/#638, 1 perf #639, 1 a11y #640); 8/8 Sonnet reviews approve; 0 abandons, 0 circuit-breaks

DEEP AUDIT was DUE (last standalone sweep run 89, 2026-07-19; 4 days + runs 90/91 since — past the ~4-run/~24h cadence). Ran a 7-lens read-only Haiku fan-out over the WHOLE repo BEFORE scouting: correctness/dead-code, security/RLS/Track-G, design-bar/a11y, perf, test/eval coverage, artifact-freshness, and mobile/functional-reality. Own maker spot-checks first: typecheck 6/6 green, 1060 core tests pass +27 skipped, production web build exit 0 no missing-export, validation 8/8 READY 0 unmet, prices 499/3999/999/7999¢ byte-consistent, BUSINESS_CASE_SUMMARY valid YAML (base $33,450 == body, floor_met false), 0 open PRs, clean tree.

**FLAGSHIP finding — a CRITICAL dead-on-arrival bug on the native mobile Pantry screen** (mobile lens). The screen read `{ id, quantity, unit, daysUntilExpiry }` but `/api/mobile/pantry` returns `getPantryView` rows (`{ canonicalItemId, name, baseQtyOnHand, status, estimatedRunOutAt, … }`). Across the untyped JSON boundary TypeScript couldn't catch it, so every pantry item rendered a blank quantity, an undefined React key, and "undefinedd left". Two real mobile correctness bugs + one hot-path perf win + one a11y clear shipped this run — all file-disjoint.

### PR #638 — fix(mobile): reconcile the pantry screen with the real API shape (dead-on-arrival list) [correctness / core-loop / mobile]
Reconciled the two shapes in ONE pure, tested place: `toMobilePantryItems` in `@gm/core/pantry` maps a pantry-view row → `{ id: canonicalItemId, name, quantity: Math.round(Number(baseQtyOnHand)), status, runsOutInDays }`. The on-hand figure mirrors the web pantry's existing "N on hand" idiom (Drizzle surfaces `numeric` as a string; a bad value coerces to 0, never NaN); the countdown is derived from `estimatedRunOutAt` and RELABELED from "Expires" to "Runs out" — that field is a consumption run-out PREDICTION, not a spoilage/expiry date, so the old copy overclaimed (a "no fake data" honesty fix). Keyless guard: `mobile-view.test.ts` (8 cases: id/quantity mapping, string+NaN coercion, ceil/sign of run-out days, ISO-string boundary, null). Gate green (1060 core tests, typecheck 6/6, prod build exit 0, mobile tsc). 2/2 Sonnet APPROVE — both independently ran the tests/typecheck/build and confirmed the shape reconciliation, tenancy path (`verifyMobileToken`+`rateLimit`+`withTenant` untouched), and copy honesty.

### PR #637 — fix(mobile): gate the onboarding-finish redirect on the real API outcome [correctness / side-effect-integrity / mobile]
The onboarding step-3 "finish" awaited `apiFetch(…"finish")` then called `router.replace("/")` UNCONDITIONALLY. `apiFetch` returns a Response and only rejects on network/timeout — a 5xx resolves normally — so a server-side failure of the very call that marks onboarding complete still sent the user home while onboarding stayed unmarked, bouncing them back into onboarding on next launch. Gated the redirect on `res.ok` (surfacing the same retry error the network path shows). Matches the `res.ok` idiom already used across the other mobile screens (`pantry`, `capture`, `profile` delete-account). Mobile tsc green. 2/2 Sonnet APPROVE — verified `apiFetch` resolves on 5xx, `finally` still runs on early return, consistent with the file's own conventions.

### PR #639 — perf(pantry): dispatch the two independent reads in reprojectStock together [perf / hot-path]
`reprojectStock` awaited the ledger events, then issued the item shelf-life read — two independent reads. Dispatched together with `Promise.all`. reprojectStock is on the hottest write path (once per purchase line item during ingestion, once per recipe-cook ingredient). Reviewer A traced the porsager/postgres transaction queue and confirmed concurrent statements on a tx-scoped handle serialize FIFO — SAFE, and an established pattern here (`ask/actions.ts`, mobile `wrapped`/`spend`). Behavior-preserving; existing persist unit (10) + integration tests are the guard (a new test would only assert Promise.all returns a 2-tuple in order = padding). MAKER SELF-CORRECTION before opening the PR: Reviewer A's precise non-blocking note — inside a transaction postgres.js still does two serialized wire round-trips, so the win is removing the JS-side await STALL, not saving a round-trip — was folded into the comment + commit message + PR body for accuracy (amend + force-with-lease, pre-PR). 2/2 Sonnet APPROVE.

### PR #640 — a11y(ask): raise the starter-prompt chips to the 44px touch target [design/a11y]
The `/ask` starter-prompt chips render as interactive `<button>`s but use `.pill-brand` (→ `.pill` = `px-2.5 py-1 text-xs`, ~24px tall) — below the WCAG 2.5.5 / Apple-HIG 44px minimum on a mobile-first PWA. Added `min-h-[44px] px-4` inline (the established idiom from #627/#628/#630/#631), leaving the shared `.pill` class untouched so the seven non-interactive `pill-brand` display badges stay compact. eslint --max-warnings=0 clean, web build green. 2/2 Sonnet APPROVE — confirmed `.pill` is `inline-flex items-center` (centered 44px target), the `px-4` utility cleanly overrides the component-layer `px-2.5`, single-line diff, only the interactive chip affected.

### Rejected / deferred (with evidence)
- **ingest.ts:99/155 `purchaseRows[0]!.id` / `lineRows[0]!.id`** (correctness lens, flagged "CRITICAL ledger corruption") — FALSE POSITIVE. Drizzle `.returning()` on a single-row INSERT yields exactly one row or throws on failure (the transaction rolls back) — it never returns an empty array, so the non-null assertion is safe; no partial-ledger state is reachable.
- **CORS `Access-Control-Allow-Origin` "missing"** (security lens) — NOT a hole. Omitting ACAO is the SECURE same-origin default; ADDING `*` would be the bug. The finding was a comment-clarity nit, not a fix to make.
- **Stripe webhook `metadata.userId` not UUID-validated** (security lens) — LOW / already fails safe. metadata is stamped server-side by our own checkout; a malformed value only makes the entitlement write throw, which the outer try/catch logs and returns 200 for (no crash, no corruption). Defensive-only, below the bar.
- **Multi-instance in-memory rate-limit / LLM-quota / demo-spend ceiling** (security lens) — REAL at scale but OWNER-GATED, already tracked (PENDING_OPS `llm-quota-redis-upgrade`, surfaced in LAUNCH Step 10 run 91); the Redis fix is un-provable keyless in CI (degrade path stays in-memory) → correctly deferred.
- **log-cook.ts sequential `appendLedgerAndReproject` loop / ingest.ts N+1** (perf lens) — DEFERRED. Each ledger append reprojects from the accumulated ledger, so the appends are order-dependent, not safely parallelizable; the ingest normalization N+1 is a large, risky refactor on a non-latency-critical background path (runs 38/41 precedent).
- **depletion.ts zero-delta-confirmation aging / embedding zero-vector normalization / recipe-import multi-Recipe** (correctness lens) — by-design or negligible-impact; below the bar.

### FOLLOW-UPS noted (future runs)
- **Mobile pantry `item.name` rendered raw (no titleCase)** — both #638 reviewers flagged it; PRE-EXISTING (the old screen also showed raw names) and NON-BLOCKING. `titleCase`/`humanize` live only in `apps/web/app/lib/format.ts`, unreachable from `apps/mobile` (React Native, excluded from the pnpm workspace) — folding it into the core mapper would introduce a layering smell. A future change would add a tiny mobile-local title-caser (or a shared @gm/shared formatter) if raw canonical names prove non-title-cased in practice.
- **`/digest` `<a className="pill-brand">` (apps/web/app/page.tsx:291)** — an interactive, sub-44px `pill-brand` LINK (not a display badge); the next a11y touch-target candidate after #640 (out of scope for the #640 single-chip diff).
- **No outcome-asserting test across the mobile JSON boundary** — the #638 pantry bug slipped because the mobile app has no e2e/contract harness (the web `journeys.spec.ts` is web-only). The pure core mappers #638 introduced are the right seam: a future Track-F change could contract-test each mobile-API DTO against its core mapper keyless, catching shape drift before it reaches a device.

**LESSON — an untyped client↔server JSON boundary hides shape drift; reconcile it in ONE pure, tested place.** The mobile pantry read fields the API never sent, and nothing caught it because the boundary is a `res.json() as T` cast on the client and a `Response.json(rows)` on the server — TypeScript sees neither side of the other. The durable fix isn't per-screen patching; it's a pure mapper in `@gm/core` that owns the DTO shape and is unit-tested, so the contract lives in one keyless-guarded place both surfaces depend on.

**PROCESS LESSON — a maker-owned accuracy correction before opening the PR is not a review cycle.** Reviewer A's precise "round-trip vs await-stall" note on #639 was factually correct and non-blocking; since I own the file and no PR existed yet, I amended the comment/commit/PR framing (force-with-lease) rather than shipping an overstated claim — the same honesty discipline the run-91 reachability re-framing enforced, applied pre-emptively.

**DEEP AUDIT 2026-07-23 (run 92) summary:** 7-lens Haiku sweep. Mobile/functional-reality → the flagship CRITICAL pantry-contract bug + the onboarding side-effect-integrity bug (both shipped #638/#637). Correctness → the two above; ingest `!` "critical" = false positive. Perf → reprojectStock parallel reads shipped (#639); log-cook/ingest N+1 deferred with evidence. Design/a11y → /ask chips shipped (#640); /digest link noted as next. Security/RLS/Track-G → NO new live holes (RLS on all public tables, constant-time auth, captcha, CSP/HSTS/nosniff, Stripe constructEvent + timing-safe webhooks, server-side entitlements); the multi-instance in-memory ceiling stays owner-gated. Artifact-freshness → prices/features/models all match code; the only note is the privacy-disclosures owner-email placeholder (already tracked in ACCEPTANCE_AUDIT as an owner step). Test coverage → strong; the real gap is the mobile-boundary contract-test seam (follow-up noted). Monetization → NO buildable non-reach lever (reach-gated #190 re-confirmed, ~10th consecutive run).

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM; no buildable non-reach lever, independently re-confirmed). Confidence statement stays UNCHECKED. validation 8/8, 0 unmet; ship gate MET (A). A productive convergent run — a DEEP AUDIT that caught a genuine CRITICAL dead-on-arrival mobile bug plus a second mobile side-effect bug, a hot-path perf win, and an a11y clear; 8/8 first-pass Sonnet approvals, 0 abandons.

## Run 93 — 2026-07-24 — no DEEP AUDIT due (run 92 swept 7 lenses <24h prior); focused 3-Haiku scout advancing the run-92 follow-ups + 3 file-disjoint clears (#642 mobile pantry names, #644 recipes 44px a11y, #643 extract @gm/core/format + web/mobile unify); 3 correct REQUEST_CHANGES (converged), 1 honest abandon (#645)

Baseline green at run start (validation 8/8 `--readiness` READY 0 unmet; prices 499/3999/999/7999¢ byte-consistent; SUMMARY valid YAML base $33,450/floor_met false; 0 open PRs; clean tree). A DEEP AUDIT was NOT due — run 92 (2026-07-23) ran a full 7-lens sweep < 24h prior. Ran a focused **3-Haiku read-only scout** (a11y/design-taste · mobile functional-reality · core Track-F coverage) to advance run-92's recorded follow-ups (mobile raw-slug names; the sub-44px cook-loop links; the mobile-boundary contract seam). Shipped the value-bar-clearing, file-disjoint work; abandoned one low-value change on a correct review objection.

**Shipped (3, file-disjoint):**
- **#642 — `fix(mobile)`: title-case pantry item names in the DTO mapper [design / "never show raw slugs"].** Canonical names are stored lowercase for matching; the web app title-cases at render, but the native Pantry screen rendered `item.name` raw ("organic hass avocados", "WHOLE MILK"). Fixed in the ONE pure, tested seam — the server-side `toMobilePantryItems` mapper that owns the mobile DTO — so the untyped JSON boundary never carries a raw slug. +3 keyless guard cases. 2/2 first-pass.
- **#644 — `a11y(recipes)`: raise the inline Remix + Add-missing actions to 44px.** The recipe-card cook-loop row paired a `min-h-[44px]` "Cook →" with two `text-xs` inline controls with no min height (the "Remix" link ~28px + the "+ Add N missing to list" submit). Both raised to `min-h-[44px] px-1` (WCAG 2.5.5 / Apple HIG) — the established cook-loop idiom. 2/2 first-pass.
- **#643 — `refactor(format)`: extract titleCase into @gm/core, unify web+mobile, title-case the remaining mobile names.** Moved `titleCase` into `@gm/core/format` as the single source of truth: the pantry DTO mapper reuses it (dropping its inline copy, −16 lines), the three remaining raw-name native screens (`list.tsx` card, `use-it-up.tsx` at-risk banner, `spend.tsx` top-items) title-case at render via the `@gm/core/*` alias, and `apps/web/app/lib/format.ts` re-exports it (dropping its own copy — its 14+ consumers unchanged). +6-case unit test. 2/2 after 2 rework cycles (below).

**Abandoned (1, clean tree):**
- **#645 — `a11y(cook)`: "Remix this recipe" 44px — ABANDONED on a correct review objection.** Reviewer A showed the naive `min-h-[44px] items-center` adds ~12px of centering space above/below the ~20px text, so the `mt-2→mt-1` nudge doubled the visible gap and pushed the CookMode section down. The repo's correct idiom is a negative-margin offset (`pantry/page.tsx:408`, `save-button.tsx:45` use `-my-2 -mr-2`) that cancels the height growth — but sizing it right is layout-sensitive and there is no practical headless-browser check for a 4px gap. On a low-value single-link change, the disciplined call was to abandon rather than iterate blind. Re-queued as a follow-up (with the same screen's `back-link`/`nav-link` controls Reviewer B flagged), for a run that can verify layout in a browser.

**Review cycles (all correct, all converged — maker≠checker working, not churn):**
- #643 v1 (a mobile-local `apps/mobile/lib/format.ts` copy) → Reviewer B REQUEST_CHANGES: avoidable duplication, the native app CAN import `@gm/core` via the tsconfig alias (it already does for billing/recipe/personalization). Reworked to `@gm/core/format`.
- #643 v2 (core copy, mobile+mapper use it) → Reviewer B REQUEST_CHANGES: the "single source of truth" docstring was false while `apps/web/app/lib/format.ts` kept its own identical copy. Made the web lib re-export from `@gm/core/format` → genuinely one implementation → 2/2.
- #645 → Reviewer A REQUEST_CHANGES (the 44px layout-shift correctness point) → abandoned.

**Lessons (also in LOOP_MEMORY):** (a) a workspace-excluded native app CAN still import pure `@gm/core` logic via the `@gm/core/*` tsconfig path alias — reuse the shared module rather than hand-copy a display rule (drift risk); (b) a "single source of truth" claim is only true once EVERY copy (including the web lib's) routes through the shared module; (c) don't ship a layout change you can't visually verify on a design-bar repo — abandon (partial batch is fine) rather than iterate blind.

**Follow-ups (future runs):** #645's cook-screen "Remix this recipe" + `back-link`/`nav-link` 44px (needs the negative-margin idiom, browser-verified); the mobile JSON-boundary contract-test seam (open since run 92); optionally move `humanize` into `@gm/core/format` too if a native screen ever needs enum humanization.

**Monetization:** NO buildable non-reach lever (reach-gated #190 re-confirmed, ~11th consecutive run — every pricing/tier/conversion/retention/margin lever is built; the gap to the $100K floor is a download multiplier = owner GTM).

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM; Confidence UNCHECKED). validation 8/8, 0 unmet; ship gate MET (A). A productive convergent run — completed the mobile "never show raw slugs" story (4 screens now match the web surface via one shared formatter) + 2 cook-loop touch-target clears; the review cycles improved every change they touched.

## Run 95 — 2026-07-26 — DEEP AUDIT (6-Haiku lens sweep, ~24h since run 94) + 6 file-disjoint clears incl. the T1–T3 trial-email revenue lever (#658); 2-Sonnet review per change (1 REQUEST_CHANGES on #654 → abandoned; 1 stale-base REQUEST_CHANGES on #658 → rebased → APPROVE); 1 disciplined abandon (#654)

**Baseline green at start:** typecheck 6/6, @gm/core 1067 tests +27 skip, prod web build exit 0 (no missing-export), validation 8/8 READY 0 unmet, prices 499/3999/999/7999¢ byte-consistent, SUMMARY base $33,450/floor_met false, 0 open PRs, clean tree.

**DEEP AUDIT 2026-07-26** (6 Haiku read-only lenses over the whole repo — DUE ~24h since run 94): **SECURITY/RLS/Track-G** — NO new holes (all public tables RLS+policy; ~41 routes auth/rate-limit/zod/error-hygienic; timing-safe Stripe/RevenueCat/Gmail/cron webhooks; login lockout no-enumeration; entitlements server-side; no committed secrets). **CORRECTNESS** — 2 real error-hygiene gaps (#652, #653). **DESIGN/a11y** — 3 real 44px touch-target gaps (#655, #656, #657). **PERF** — cook-tonight double-`.find()` (candidate, later abandoned as marginal). **MONETIZATION** — the reach-gated conclusion was adversarially CHALLENGED and PARTLY FALSIFIED: one genuine buildable non-reach conversion lever, the trial-ending **T1–T3** email sequence, was documented in `docs/brand/EMAIL_LIFECYCLE.md` but NEVER built while H14/H15 were → BUILT this run (#658). **MOBILE+STORE** — parity CLEAN (18 screens vs `/api/mobile/**`); the one store finding (Play contact-email personal) is owner Human-Core, already flagged.

**SHIPPED (6):**
- **#652 (fix/mobile error-hygiene)** — `/api/mobile/recipes` GET awaited the `withTenant` DB read with NO try/catch (the last unguarded mobile route) → a DB blip surfaced as a raw 500 with stack/schema-leak risk instead of the calm `serverError()` JSON every sibling returns. Wrapped. 2/2 first-pass.
- **#653 (fix/instacart error-hygiene)** — the reorder DB read (`loadReorderInputs` + `getActiveListView`) sat OUTSIDE the route's try (which only wrapped the Instacart API call) → uncontrolled throw. Own try/catch with an accurate message; Instacart-API catch untouched. 2/2 first-pass.
- **#655 (a11y/capture)** — the icon-only dictate mic button was `btn-sm !px-2 !py-2` (~28–32px); raised to a 44px target (`!p-0 min-h/w-[44px]`, icon centered via the `.btn` base, moved to the `right-2 top-2` corner). 2/2 first-pass.
- **#656 (a11y/getting-started)** — onboarding Dismiss button `btn-sm`→`min-h-[44px]`. 2/2 first-pass.
- **#657 (a11y/share)** — the 3 copy-link buttons (invite / household / cookbook) `btn-sm`→`min-h-[44px]` (one coherent PR, 3 disjoint files). 2/2 first-pass.
- **#658 (feat/lifecycle T1–T3 — the headline revenue lever)** — the trial-ending email sequence: H16 welcome (day-0, lists what's unlocked + connect-Gmail), H17 ~2-days-left transparency, H18 expiry-day annual-upsell, built on the SAME `runLifecycleCampaign` orchestrator as H14/H15. Key architectural finding: the Stripe webhook did NOT distinguish `trialing` from paid (it collapsed both to `entitlement=premium`), and `subscription_renewal_at` stored the event time, not trial-end — so the build added ADDITIVE `subscription_status` + guarded `trial_end_at` signals (webhook untouched otherwise) + three windowed idempotent candidate queries (>3d / 1–3d / ≤1d left, status='trialing') + 3 cron routes + experiments + `vercel.json` crons + the `lifecycle-trial-emails` keyless capability + 18 unit tests. **Billing-accuracy crux:** the trial uses Checkout `trial_period_days` with the default `payment_method_collection:"always"`, so a card is on file and the trial AUTO-CONVERTS unless cancelled → copy is TRANSPARENCY + annual-upsell ("begins"/"continues"/"cancel anytime"), NEVER a false "buy now to keep premium"; the only saving offered is the genuinely-cheaper annual plan (no coupon). Reviewer A empirically stood up local Postgres and verified all three query windows (contiguous, non-overlapping, boundary-inclusive-correct); Reviewer B verified copy accuracy + no gamed numbers. 2 reviewers APPROVE (after a rebase resolved a stale-base diff artifact).

**ABANDONED (1, Reviewer-B value rejection):** **#654 (perf/cook-tonight Map)** — replacing two `full.find()` scans per row with a Map matched the sibling `discover` exemplar, but at n≤8 (both `ranked` and `full` capped at 8) the perf win is negligible and the value is purely stylistic parity → Reviewer B REQUEST_CHANGES as marginal; abandoned cleanly. The maker≠checker guard correctly filtering a sub-bar micro-perf (consistent with run 94's deferral of similar sequential-await items).

**LESSON — the "every lever is built" claim needs an ADVERSARIAL re-check, not inheritance.** Run 94 (and ~11 runs prior) recorded "NO buildable non-reach lever — every conversion/retention lever is built." This run's adversarial monetization lens (a Haiku told to CHALLENGE that conclusion) found it was FALSE: the trial-ending sequence was documented-but-unbuilt. The reach-gated conclusion is TRUE for the floor (downloads is the dominant lever), but "every lever is built" was an over-inherited claim. Re-derive the monetization completeness claim adversarially each deep audit rather than carrying it forward.

**LESSON — a documented-but-unbuilt lever hides behind a plausible "it's basically infra" estimate.** The scout estimated T1–T3 as "4 files, existing infra." Reality: the existing webhook didn't track trial STATUS distinctly (trialing collapsed to premium), so the build needed additive webhook plumbing first. Always trace the actual DATA the feature needs to its source before sizing it.

**Monetization:** the trial-ending conversion lever is now BUILT (#658); median deliberately UNMOVED (no adoption banked — measured live per the anti-gaming rule, same as H14/H15/referral). Floor remains reach-gated (#190, base ≈ $33K < $100K = owner-GTM). No other buildable non-reach lever identified this run.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190; Confidence UNCHECKED). validation 9/9, 0 unmet; ship gate MET. A high-value convergent run — the DEEP AUDIT earned its keep (6 real clears incl. a genuine revenue lever the loop had been wrongly recording as "already built"), and the 1 abandon is the value-bar guard working.

**Known minor follow-up (recorded, not blocking):** a user who CANCELS during the trial (`cancel_at_period_end` set while `sub.status` is still `trialing`) has a latest `subscription_status='trialing'`, so they can still receive T2/T3. Low severity (the emails still carry "cancel anytime — no charge"; not a false-charge claim). Future refinement: record `cancel_at_period_end` (or treat it as a distinct status) in the webhook and exclude such users from the T-queries.

## Run 96 — 2026-07-27 — no DEEP AUDIT due (run 95 swept 6 lenses <24h prior); focused scout sweep + 1 file-disjoint Track-F clear (#660 kill a coverage-run flake in the margin-eval guard); 2-Sonnet review APPROVE first-pass; 2 a11y candidates held (layout-sensitive, unverifiable blind — same discipline as runs 93/95)

**Baseline:** typecheck 6/6, prod web build exit 0 (no missing-export), validation 9/9 READY 0 unmet, prices byte-consistent (499/3999¢), SUMMARY base $33,450/floor_met false, 0 open PRs, clean tree — EXCEPT the core suite showed **1 intermittent failure** in `src/llm/evals/margin/workflows.test.ts` (the run's headline finding).

**SCOUT (2 Haiku lenses — correctness/security + design/a11y/artifact; no full deep audit, not due):** correctness/security came back **CLEAN** (all public tables RLS+policy; every mutable/expensive route rate-limited + zod-validated + error-hygienic; timing-safe webhooks; 8s timeout on the Plausible fetch; no auth-bypass; the only notes were already-known/owner-gated). design/a11y found **2 real 44px gaps** but both are the layout-sensitive shared-CSS `.back-link`/`.nav-link` (`globals.css`, 29+ call sites) + the cook-screen "Remix this recipe" link (`cook/[id]/page.tsx:151`) — the SAME items runs 93 (#645) and 95 disciplined-abandoned because the negative-margin sizing is layout-sensitive and there is no practical headless check for a 4px gap. Booting the full app against a seeded DB + auth to verify a nav-link nudge is disproportionate; shipping blind violates the standing LESSON ("don't ship a layout change you can't visually verify on a design-bar repo"). **HELD** — a quiet coherent run is a success.

**SHIPPED (1):**
- **#660 (test/margin-eval — Track F flake fix)** — the margin multi-workflow eval guard's two `async` tests (the file's only ones) each `await runWorkflowEval`, yielding the event loop per case. Their wall-clock is dominated by event-loop SCHEDULING, not real work: the fake `run` is trivial and `emitEvalOutcome` → `margin-meter@0.2.0` returns before any network I/O with no `MARGIN_INGEST_KEY`. Under the parallel `--coverage` run the "grades every case…" test climbs from ~385ms isolated to ~1s under load, and a rare CPU-starvation spike crossed vitest's **5000ms** default (the observed failing file duration was **5020ms**) → an intermittently-red REQUIRED core-test check that would block the whole auto-merge pipeline. Fix: an explicit **20 000ms** budget on both async tests (pure starvation headroom — NO assertion changed; a true hang still fails at 20s). Ruled OUT an env race: vitest `isolate:true`/`pool:forks` keeps `process.env` per-file and the only `MARGIN_WORKFLOW_ID` writer (`runner.ts`) restores it in an unconditional `finally`. Consistent with sibling `*.eval.test.ts` files that already set 120–180s per-test overrides. Reviewer A independently stood up the reasoning against `runner.ts` + the installed `margin-meter@0.2.0` transport + the vitest config; both Sonnet reviewers APPROVE first-pass.

**LESSON — a green baseline is not a green baseline until you read the FULL test summary line, not just the coverage table.** The core suite's coverage table scrolls past the pass/fail count; the flake only showed as `1 failed | 1085 passed` on the summary line. Grep `Tests |Test Files ` on the baseline run BEFORE concluding "baseline green" — a single-line intermittent failure is exactly the kind of thing a truncated tail hides.

**LESSON — diagnose a flake to a ROOT before "fixing" it; a timeout number that matches the default IS the tell.** The failing file duration (5020ms) sat ~20ms over vitest's 5000ms default per-test timeout — that near-exact match, plus "passes isolated / slow under the parallel coverage run / high variance," is the signature of CPU-starvation on an async test, NOT a logic bug. Verified the alternative hypotheses (env cross-file leak, network hang, missing await) were all false before touching anything, so the fix is a timeout bump (correct) rather than a masked deadlock.

**Monetization:** unchanged — floor remains reach-gated (#190, base ≈ $33K < $100K = owner-GTM); the T1–T3 lever built in run 95 stands; median deliberately unmoved (measured live, anti-gaming). No buildable non-reach lever surfaced this run.

**Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190; Confidence UNCHECKED). validation 9/9, 0 unmet; ship gate MET. A quiet, disciplined convergent run: one genuine Track-F fix that unblocks the auto-merge pipeline, a clean security scout, and two candidates correctly held rather than shipped blind.
