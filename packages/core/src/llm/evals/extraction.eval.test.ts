...
import { aggregate, formatReport, scoreReceiptExtraction, type EvalCaseResult } from "./harness.js";
...
describe.skipIf(!RUN)("receipt extraction evals (live Gemini)", () => {
  it("meets the quality bar across golden receipts", async () => {
  ...
  it("LLM-as-judge agrees the extraction is faithful", async () => {
    const client = new GeminiClient();
