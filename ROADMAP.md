...
- [ ] **§11 marketing media-gen adapter (image + video + music + voiceover).** Build the thin media-gen adapter (GTM_STANDARD §11) so the marketing loop produces multi-format creative on the EXISTING Gemini key: IMAGE (Nano Banana), VIDEO (Gemini Omni Flash), MUSIC (Lyria 3), VOICEOVER (Gemini TTS) — a video can carry soundtrack and/or narration from the same key. Route via `getProvider`/`geminiProvider` + the Interactions API. Every asset passes the maker≠checker not-obviously-AI + FTC audit before publish; cost-governed; preview ids pinned. Stages creative pre-launch, publishes only post-§13-approval on a connected channel. Build epic: #442. **BUILD STATUS (run 62, #509): the thin ADAPTER is built — `@gm/core/media` (`MediaGenClient` + `getMediaGenClient`) generates IMAGE/VIDEO/MUSIC/VOICEOVER through the shared `GoogleGenAI` provider on the existing Gemini key, with the maker≠checker pre-publish audit gate (`auditMediaAsset`: FTC AI-disclosure + curated not-obviously-AI slop denylist), preview ids pinned in one place (`MEDIA_MODELS`), cost-governed (`MEDIA_TIMEOUT_MS`), and degrade-by-default (no key → `unavailable`; error/timeout → `error`; never publishes). Registered as the `marketing-media-gen` keyless capability. Registered as the `marketing-media-gen` keyless capability. NOTE: this RETIRES the old `gtm-connect-media-gen` owner step — generation works on the existing key; the only remaining owner step to go live is connecting the posting CHANNEL (Track H).** **BUILD STATUS (run 63, #515): the batch STAGING CONSUMER is now built on top of the adapter — `stageCreative(brief, client?, now?)` (`@gm/core/media`) takes a `CreativeBrief` (the per-format set of creative one launch beat needs) and returns a reviewable metadata-only `StagingManifest` (format/status/model/mime/audit/bytes/operationName/reason + a counts tally) plus the raw `MediaGenResult[]` (which carry the payloads) for the caller to persist at the owner-gated edge. It preserves + extends the adapter's contract: no key → all-`unavailable` manifest (no network, no throw); audit-first rejection spends nothing; a per-item guard + an exhaustive `default` arm keep an off-type/JSON-authored brief from ever aborting the batch. Proven keyless via `staging.test.ts` (injected fake provider: happy batch + format→model routing + payload-omission, no-key degrade, audit-reject, provider-error isolation, off-type-format degrade, empty brief, throw guard). Box STILL stays `[ ]` under EVIDENCE-BASED DONE — two increments remain before the §11 OUTCOME is real: (a) PRODUCING actual creative needs the owner-gated Gemini preview-model key (same boundary as the adapter); and (b) no invocation site yet AUTHORS a real `CreativeBrief` (from the brand kit / launch plan) and calls `stageCreative` on a schedule to persist staged assets for the §13 audit. The orchestration + persistence-shape half is done + keyless-proven; the remaining half is owner-key + a concrete brief-authoring consumer.** **BUILD STATUS (run 68, #544): the brief-AUTHORING consumer — part (b) above — is now built: `launch-briefs.ts` (`@gm/core/media`) hand-authors `LAUNCH_CREATIVE_BRIEFS` (deterministic `CreativeBrief[]` for the real launch beats in `docs/brand/LAUNCH_PLAN.md` — Product Hunt listing, launch-day social, waitlist email header — styled per `BRAND_KIT.md`, every prompt a concrete scene that passes the slop audit + every item carrying an FTC disclosure), `auditLaunchBriefs()` (keyless proof every authored item is audit-clean), and `stageLaunchCreative()` (the batch driver over `stageCreative` — the launch-plan invocation site, degrade-by-default: no key → all-`unavailable` manifest). Proven keyless via `launch-briefs.test.ts` (well-formed unique briefs, disclosure on every item, every item audit-clean, no-key degrade, all-`ok` via an injected fake provider); registered under the same `marketing-media-gen` capability. Box STILL stays `[ ]` — the ONLY remaining increment is part (a): PRODUCING real creative + persisting it on a schedule needs the owner-gated Gemini preview-model key + Track-H storage/channel. The entire keyless half (adapter + staging consumer + brief authoring + batch driver) is now built + proven.**

- [ ] **§34 pre-launch funnel — public demo of the core aha + gated beta.** Replace the blank waitlist with: a public, no-account, bounded + HARDENED (Track H) demo of the core aha (paste or snap a grocery receipt → watch it auto-fill a pantry list, no…) that goes live only after it clears the quality bar, driving the waitlist; then a gated-beta invite mechanism (waitlist → codes → real app, site gate up for others) yielding the first real PMF cohort. Full product stays gated; demo/beta build is autonomous + quality-gated, traffic/invites post-§13-Gate-1, public launch = Gate 2. Build epic: #453. **BUILD STATUS (both halves now built; box stays `[ ]` because the OUTCOME — a real PMF cohort — is owner-gated, not self-certifiable): Part A public `/demo` shipped run 55 (#471); Part B gated-beta invite MECHANISM shipped run 56 (#475) — `@gm/core/security/invite-code`, migration 0021, hardened `POST /api/invite/redeem` granting a DISTINCT `SITE_GATE_INVITE_SECRET`, `/join` page, `invite:issue` owner script, e2e-proven. ACTIVATION is owner-gated: apply 0021 + set `SITE_GATE_INVITE_SECRET` + issue codes to real traffic (PENDING_OPS `gated-beta-invite-codes`).**
...
- [x] **H12. Surface the already-built Family/household tier at the paywall + onboarding.** The
      ...
      _Done (run 36, PR #323): paywall half already shipped (gated Family card on `/upgrade`, PRs #154/#244).
      Onboarding half added — a "Cooking with a household?" affordance on the onboarding Done step
      ...
      case UNMOVED. 2 Sonnet reviewers APPROVE. (Relanded in the run-37 housekeeping after run-36's PR #324
      hit a shared-ledger merge conflict; the code itself has been on `main` since #323.)_
...
- [x] **Independent QUALITY GRADE = A/A+ on every ship-critical dimension** (and ≥ B elsewhere), per
      ...
      **B** dims (`tests_evals`, `performance`) non-ship-critical with named non-blocking gaps (open quality
      issues #319/#320), and NO open ship-critical `top_gap`. This closes the re-grade blocker that kept the
      box open through run 35. The loop neither authored nor influenced the grade._
