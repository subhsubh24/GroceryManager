...
## Lessons
- **2026-07-20 (run 89) — DEEP AUDIT (standalone 4-Haiku lens sweep, due since run 86 ~24h) + 2 file-disjoint a11y clears with the corrected 44px idiom (#630 cook-loop, #631 first-run/conversion CTAs); 4/4 Sonnet reviews approve (1 review cycle on #630); 0 abandons.** Baseline green at start (typecheck 6/6, `@gm/core` suite exit 0, validation 8/8 READY 0 unmet, migration 0021 intact, prices 499/3999/999/7999¢ byte-consistent, SUMMARY valid YAML base $33,450/floor_met false, ship gate MET (A), clean tree). **DEEP AUDIT 2026-07-20** (4 lenses over the whole repo): **SECURITY/Track-G CLEAN** (all tables through 0021 RLS+policy; ~41 routes rate-limit+zod+error-hygienic; constant-time auth verify #622 + lockout; per-user LLM quota + demo ceiling; captcha; CSP/HSTS/nosniff; Stripe constructEvent + timing-safe RevenueCat/Gmail/cron webhooks; entitlements server-side; no committed secrets). **CORRECTNESS: 1 candidate REJECTED (false positive)** — the public `parse-receipt` demo returns HTTP 200 `{error}` (no `items`) on a zero-item successful extraction; the SOLE client `demo/demo-client.tsx:105` guards `if (!res.ok || !data.items)` → handles it, no crash; 200-for-zero-items is a defensible distinction from the 502 error path; a 422 change is zero-behavior on the only caller → sub-bar. **MONETIZATION reach-gated RE-CONFIRMED** — pricing byte-consistent (no drift); every pricing/tier/conversion/retention/margin lever built (referral, month-3 annual nudge, win-back, Family dark behind FEATURE_HOUSEHOLDS); the ~$67K floor gap = a ~2.7–3× download multiplier = owner-GTM #190. **DESIGN/a11y: real sub-44px `btn-sm` controls → 2 surfaces shipped.** **Shipped:** **#630** — cook-loop "Cook →" (recipes + use-it-up) + shared "Cooked it" button (~28px `btn-sm`) → base `.btn` + `min-h-[44px]`. **#631** — Getting Started checklist links + PWA "Install" + Gmail "See Premium" CTA (~28px `btn-sm`) → base `.btn` + `min-h-[44px]` (install-prompt dismiss X already 44px). **LESSONS:** (1) **READ the latest LOOP_MEMORY design-idiom lesson BEFORE choosing an idiom — the chronologically-latest MERGED commit can BE the anti-pattern the same run's lesson corrects.** I first modeled #630 on #627 (which shipped `btn-sm + min-h-[44px]` and auto-merged BEFORE run-88's feedback landed) instead of the run-88 lesson + the corrected #628 (base `.btn`). Reviewer B's REQUEST_CHANGES was a CORRECT premise (not stale) → resolved by ADOPTING the fix, not overriding. When two exemplars conflict, prefer the one the LESSON endorses, not the latest-merged. Maker≠checker earned its keep: the Sonnet reviewer caught a real design-idiom regression the maker's #627-anchored taste missed. This closes the run-88 a11y follow-up for the cook-loop + first-run/conversion surfaces with the RIGHT idiom; only the staples surface remains. (2) **`staples/page.tsx` "Set" stays DEFERRED (per run 88):** it's in a `flex items-center gap-1.5` next to a ~28px number input; a correct fix needs the input raised too + a real browser visual check the headless env can't do — building it blind risks a visual mismatch. Splitting the surface (toggle now, Set later) would be incoherent → whole surface deferred to a design-polish pass that can render. (3) **A scout finding is DATA, not a verdict — verify against the actual caller:** the parse-receipt "crash" dissolved on reading the one client's `!data.items` guard. **Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM; no buildable non-reach lever, independently re-confirmed). Confidence statement stays UNCHECKED. validation 8/8, 0 unmet; ship gate MET (A). A coherent convergent run — one due deep audit (security/correctness/monetization all clean/converged) + two a11y clears with the corrected idiom.
- **2026-07-19 (run 88) — 5-Haiku scout sweep (folded deep-audit-equivalent; DEEP AUDIT NOT due — run 86 ran a standalone 6-lens sweep the same day) + 4 file-disjoint clears (#625 marketing store-honesty, #626 units Track-F, #627/#628 44px a11y); 8/8 Sonnet reviews approve (1 review cycle on #628); 0 abandons.** Baseline green at start (typecheck 6/6, 1049 core tests +27 skipped, prod build exit 0 no missing-export, self-validation 8/8 READY 0 unmet, 0 open PRs, clean tree). SECURITY/RLS/Track-G lens CLEAN (all tables through 0021 RLS+policy, ~41 routes hardened, entitlements server-side, timing-safe webhooks, no committed secrets, constant-time auth verify per #622); CORRECTNESS lens CLEAN (depletion/reorder/nutrition/recipe/verify/spend math traced, no bug). **Shipped:** **#625** — dropped the dark `FEATURE_HOUSEHOLDS` "household sharing" claims from THREE staged marketing artifacts (`docs/brand/EMAIL_LIFECYCLE.md` trial-welcome + expiry emails, `PRESS_KIT.md`, `CONTENT_DRAFTS.md`), substituting real reachable perks (AI recipe remix / unlimited Discover / advanced spend insights, all in `PREMIUM_PERKS`). **#626** — covered the MASS→VOLUME direction of `viaWaterDensity` (`units/index.ts:141`; only VOLUME→MASS was tested) → branch 95.12%→97.36%. **#627** — pantry "Still good?" resolution chips (`btn-ghost btn-sm` ~28px) → +`min-h-[44px]`. **#628** — review-queue bulk "Add to pantry" + dismiss controls (`btn-sm` ~28px) → base `.btn` + `min-h-[44px]`. **LESSONS:** (1) **The dark-FEATURE_HOUSEHOLDS honesty sweep extends to MARKETING artifacts, not just app surfaces.** Runs 78-82 gated every in-app + store-metadata surface; the STAGED launch copy (lifecycle emails, press kit, content drafts) still advertised household sharing as a live Premium perk. The store metadata already OMITS household (run-78 established that adding it would be a dark-feature risk), so email/press/content advertising it is the mirror-image drift. When gating a flag-gated feature, grep `docs/brand` + `docs/store` for the feature name too, and substitute a REAL reachable perk rather than leaving a dangling list (keeps the premium value story intact + matches `PREMIUM_PERKS`). (2) **The `btn-sm` + `min-h-[44px]` combo is an ANTI-PATTERN — gluing a 44px min-height onto `btn-sm` (`px-3 py-1.5 text-xs`) yields a tall pill wrapping a tiny 12px label.** Every established 44px control in the app scales padding AND font WITH height: `.chip-tap` (`min-h-[44px] px-4 text-[0.95rem]`), cook-mode (`min-h-[44px]` + `px-3.5/px-4 text-sm`), onboarding (`btn-primary/ghost min-h-[44px]` at base `.btn` px-4 py-2.5 text-sm). Correct fix for a sub-44px button: **DROP `btn-sm` and use base `.btn` + `min-h-[44px]`** (the onboarding-flow precedent), or use `.chip-tap` for answer-chip surfaces. NOTE the reviewer split: the pantry Reviewer B APPROVED `btn-sm min-h-[44px]` (as "quiet compact chips") while the review-list Reviewer B REJECTED the identical pattern on principle — the rejection is correct; adopt base sizing going forward. (3) **#627 shipped `btn-sm min-h-[44px]` before that feedback landed and auto-merged (2/2-approved, a11y-compliant, functional) — left as-is rather than re-touching just-merged code; recorded a design-polish FOLLOW-UP:** pantry resolution chips + `staples/page.tsx:169` "Set" (whose adjacent ~28px number input also needs raising) → adopt `.chip-tap` in a future pass with a real browser visual check. (4) **Verify the ACTUAL reachable caller of a coverage branch, not the first that comes to mind:** the units MASS→VOLUME heuristic is reached UNFILTERED via `ingest.ts` (receipt line in a mass unit vs a volumetric canonical base) — `logCook` also calls it but DISCARDS `method==="heuristic"` results, so the test comment was corrected to cite ingestion (Reviewer B catch). (5) **Check a coverage claim against the report before selecting:** a scout flagged `expiring.ts:51`'s domain-null branch as uncovered, but the file is already at 100% (the `{now}`-only call covers the no-domain path; the household soap row covers the filter) — rejected. **Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM). Confidence statement stays UNCHECKED. Security/RLS/Track-G + correctness re-confirmed CLEAN.
- **2026-07-17 (run 82) — CONVERGED run: 5-Haiku scout sweep (folded deep-audit-equivalent) + 2 file-disjoint clears (#602 manage-subscription store-honesty, #601 install-prompt 44px a11y); 4/4 Sonnet approve first-pass; 0 abandons, 0 rejections.** Baseline green at start (typecheck 6/6, 1036 core tests + 27 skipped, self-validation 8/8 READY 0 unmet, prod build exit 0, 0 open PRs, clean tree). Run 77 folded a standalone deep audit ~24-30h earlier; this run's 5-lens read-only sweep (Track-F coverage · security/RLS/Track-G · design/a11y+living-artifact · correctness/mobile-parity · monetization/store-honesty) doubled as a folded deep-audit-equivalent — 3 of 5 lenses NO-FINDINGS (security/RLS/Track-G CLEAN [all tables through 0021 RLS+policy, ~41 routes hardened, entitlements server-side, timing-safe webhooks/HMAC, no committed secrets]; Track-F coverage NO CLEAN GAP after sibling+barrel verification across recipe/reorder/pantry/units/nutrition/personalization/experiments/ingestion/capture/billing/security; correctness/mobile-parity CLEAN). **Shipped:** **#602** — `manage-subscription/page.tsx` (a reachable free-user page linked from `/profile`) advertised "household sharing" in its upgrade CTA AND rendered the Family (`premium_family`) plan card UNCONDITIONALLY while `FEATURE_HOUSEHOLDS` is off; it was the LAST un-gated surface after runs 78-81 gated `/help` (#596), the mobile paywall (#594), the landing page, and `/upgrade`. Mirrored the `/upgrade` gate — imported `householdsEnabled` from `@gm/db`, computed `householdsLive`, dropped the perk copy (`{householdsLive && ", household sharing"}`) and filtered `premium_family` from the plans grid when dark. **#601** — the PWA install-prompt (`install-prompt.tsx`) dismiss button was `btn-ghost btn-sm` (~28px tall, below the 44px touch-target bar); switched to the established banner-dismiss idiom `flex h-11 w-11 items-center justify-center` (the #584 Gmail-banner / theme-toggle precedent) with neutral `hover:bg-ink-50`, `aria-hidden` on the icon. **LESSONS:** (1) **The dark-FEATURE_HOUSEHOLDS honesty sweep took FIVE runs (78→82) to reach every surface — the durable fix is to grep the WHOLE `apps/web/app` tree (+ mobile + docs/store) for the feature name in ONE pass and gate every hit at once, rather than discovering one straggler surface per run.** The surfaces gated across runs: mobile paywall perk-list (#594), web landing `premiumFeatures` (run 78-era), `/upgrade` perk + Family card, `/help` FAQ (#596), and now `manage-subscription` CTA + Family card (#602). Both the CTA copy AND the tier CARD are separate dark-feature vectors on the SAME page — a scout must check both the perk sentence and the plans grid. Future flag-gated features should ship the gate on ALL surfaces in the introducing PR. (2) **`manage-subscription`'s plans grid rendered the Family tier card unconditionally because its price-display was tier-specific (only `premium_monthly`/`premium_annual` show a price), so the Family card silently rendered as a bare "Family" label** — a card with no price is easy to miss in a design review but still advertises a dark tier. The gate is a `.filter((p) => p.tier !== "free" && (householdsLive || p.tier !== "premium_family"))` — the exact shape `/upgrade` uses. (3) **For a spacious BANNER dismiss control, the visible `h-11 w-11` idiom (#584) is correct; the negative-margin hit-slop rule (run-40) is only for DENSE grid controls** where a visible 44px square would crowd the layout. install-prompt is a floating card banner (like the Gmail banner), so `h-11 w-11` matches precedent — don't reflexively apply hit-slop. Use the neutral `hover:bg-ink-50` (theme-toggle) when the surface is neutral, not the brand-tinted `hover:bg-brand-100` the Gmail banner uses. **Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM). Confidence statement stays UNCHECKED. Security/RLS/Track-G re-confirmed CLEAN.

- **2026-07-17 (run 81) — CONVERGED run: 4-Haiku scout sweep (folded deep-audit-equivalent) + 3 file-disjoint clears (#596 help store-honesty, #597/#598 Track-F coverage); 6/6 Sonnet approve first-pass; 0 abandons.** Baseline green at start (typecheck 6/6, 1036 core tests + 27 skipped, self-validation 8/8 READY 0 unmet, prod build exit 0, 0 open PRs, clean tree). Run 77 folded a standalone deep audit ~24h earlier; this run's 4-lens read-only scout sweep (security/RLS/Track-G · design/a11y+living-artifact · Track-F coverage · correctness/mobile/dead-code) doubled as a folded deep-audit-equivalent. Security/RLS/Track-G lens NO-FINDINGS. **Shipped:** **#596** — the Help & FAQ page (`apps/web/app/help/page.tsx`) documented household sharing as a live Premium feature (a Q&A, a feature bullet, a troubleshooting mention) UNCONDITIONALLY while `page.tsx` + `/upgrade` already gate every household reference behind `householdsEnabled()` (`FEATURE_HOUSEHOLDS`, off); gated all three refs to match. **#597** — added the first `reason`-copy coverage on `assessOrderReadiness` (home reorder verdict): the `urgentCount>1` plural and `count===1` singular building branches were never asserted. **#598** — covered `estimateEffort`'s no-instructions path (`instructions ?? ""` + false arm of `instr.trim() ? 1 : 0`), reachable via `semantic-layer.ts` provider recipes. **LESSONS:** (1) **The "dark-feature honesty" sweep must include EVERY user-facing content surface, not just the paywall/landing/store-copy — `/help` (and any FAQ/docs page) is a store-reviewed surface too.** Runs 78-80 gated the mobile paywall (#594), the web landing `premiumFeatures` filter, `/upgrade`, and the ASO/store docs, but never swept `/help` — which still documented household sharing as a live Premium feature in THREE places (a Q&A, a Premium bullet, a sync-troubleshooting mention). When a feature is flag-gated (`FEATURE_HOUSEHOLDS`), grep the WHOLE `apps/web/app` tree for the feature name and confirm every hit is either gated or an unrelated word (here line 189 "groceries, household, etc." is a grocery CATEGORY, correctly left) — a help/FAQ page advertising a dark feature is the same Apple 2.3.1 risk as the paywall doing it. (2) **`householdsEnabled()` is safe to call in ANY server component including statically-generated ones** — it's a pure `process.env.FEATURE_HOUSEHOLDS === "1"` read (no DB, no await), so gating a static help page on it bakes in the build-time flag value exactly like the flag is meant to work (owner sets env + redeploys). It's already the established idiom (`page.tsx`, `upgrade/page.tsx` both `const householdsLive = householdsEnabled()`); reuse the same variable name. (3) **The first assertion of a user-facing interpolated string is legitimate Track-F value even when the branch is "just" pluralization** — `assessOrderReadiness.reason` is rendered on the home screen (`page.tsx:346`), so a pluralization regression ("2 item runs out") ships a visible polish defect; pinning the exact copy (byte-verified em-dash/apostrophes) for the plural + singular branches is mutation-sensitive coverage of reachable output, not padding. (4) **A "silent fallback" finding is only real if the code is ACTUALLY silent — read the site before selecting.** A correctness scout flagged `getAdminDb()`'s `DIRECT_DATABASE_URL ?? DATABASE_URL` fallback as a silent prod-auth footgun, but `client.ts:47-56` already logs a loud, detailed `console.error` naming the exact cause AND carries an extensive rationale comment; the author deliberately chose log-not-throw to keep the dev/CI single-URL fallback working (the run-42 CI carve-out). Converting to a prod-throw is marginal over the existing loud log and risky on the auth path — REJECTED. The maker's job is to verify the scout's characterization ("silent") against the real code, not relay it. **Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM). Confidence statement stays UNCHECKED. Security/RLS/Track-G re-confirmed CLEAN.

- **2026-07-17 (run 80) — QUIET CONVERGED run: 6-Haiku scout sweep (folded deep-audit-equivalent) + 2 file-disjoint clears (#594 mobile store-honesty, #593 Track-F coverage); 4/4 Sonnet approve first-pass; 0 abandons.** Baseline green at start (typecheck 6/6, 1036 core tests + 27 skipped, self-validation 8/8 READY 0 unmet, prod build exit 0, 0 open PRs, clean tree). Deep-audit timing borderline (run 77 folded one ~18–24h earlier); the 6-lens read-only sweep across the whole repo doubled as a folded deep-audit-equivalent — 4 of 6 lenses CLEAN (security/RLS/Track-G, design/a11y/taste, living-artifact/pricing, correctness/dead-code — all NO FINDINGS). **Shipped:** **#594** — the native paywall `upgrade.tsx` mapped the full shared `PREMIUM_PERKS` unconditionally, advertising the dark "Household sharing" perk (behind `FEATURE_HOUSEHOLDS`, no native surface); mirrored the web upgrade page's `householdsEnabled()` filter with a static `MOBILE_PERKS = PREMIUM_PERKS.filter(feature !== "household")`. **#593** — added the untested no-hint else-arm of `buildExtractionPrompt`'s `retailerHint` ternary (real traffic: the public parse-receipt demo calls `extractReceipt` with no hint). **LESSONS:** (1) **The "dark-feature trap" recurs in the perk BULLET LIST, not just the tier CARD — and mobile is the higher-risk surface.** Run 79 rejected an always-visible mobile Family *card*; this run found the same dishonesty one layer down: the web `upgrade/page.tsx` filters the `household` *perk* out of `PREMIUM_PERKS` via `householdsEnabled()` (store-acceptance honesty, Apple 2.3.1), but the mobile paywall rendered the raw list. When a shared `PREMIUM_PERKS`/perk-list is rendered on BOTH platforms, check that mobile applies the SAME dark-feature filter web does — the store-reviewed native surface is where an unreachable-perk claim is most likely to draw a 2.3.1 rejection. (2) **Mobile can't read server env flags — a static filter (with a comment) is the CORRECT dependency, not a smell.** `householdsEnabled()` reads `process.env` server-side; the Expo/RN client bundle has no channel to it and no mobile API surfaces it. More importantly the binding constraint isn't the flag — mobile has no household SCREEN even if the flag flipped — so gating on a hand-maintained static list (updated alongside the future native household UI) is more correct than wiring a flag round-trip that would still lie. Don't reflexively "fix" a static feature-gate on mobile into a server round-trip. (3) **"Advertised-but-missing on mobile" splits into two kinds — DARK vs LIVE — and only DARK is a clean filter.** `household` is 100% dark on both platforms → filter it (hides nothing obtainable). `gmail_import` (same run, flagged by both reviewers) is LIVE — a premium user connects Gmail on web and the synced pantry benefits their shared account on mobile, so the value IS deliverable; filtering it would understate real value, and building native Gmail OAuth is a large/risky slice. Deferred as a considered judgment call (filter vs. deep-link-to-web connect path), NOT an oversight — recorded so a future mobile scout doesn't re-file it as a fresh bug. (4) **A single-ternary else-arm IS legitimate Track-F coverage when a real caller hits it** — verify the reachable caller (here the public parse-receipt endpoint passes no hint) and mutation-sensitivity before shipping; both Sonnet reviewers independently traced the caller graph and confirmed. **Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM). Confidence statement stays UNCHECKED. Security/RLS/Track-G re-confirmed CLEAN.

- **2026-07-16 (run 79) — 6-Haiku scout sweep + 3 file-disjoint clears (1 mobile-premium-parity FLAGSHIP #591 + 2 Track-F coverage #589/#590); 6/6 Sonnet approve, remix took 1 review cycle (2 real bugs fixed), 0 abandons.** Deep audit not due (run 77 folded one <24h earlier). Baseline green at start (typecheck 6/6, 1036 core tests + 27 skipped, self-validation 8/8 READY 0 unmet, prod build exit 0, 0 open PRs, clean tree). 4 of 6 lenses CLEAN (security, design, living-artifact, correctness — all NO FINDINGS). **Shipped:** **#591** (FLAGSHIP) — the mobile app advertised "AI recipe remix" in its paywall but had no way to REACH it (the web `/remix/[id]` page existed; mobile didn't). Built full parity: a backend `GET /api/mobile/recipes/[id]/remix` (server-side entitlement + AI quota + rate-limit + keyless-first `suggestRemix` + degrade), a native `app/remix/[id].tsx` screen (4 axis tabs + swap cards + all real states), a cook-screen entry button, and a `Remix` icon. **#589/#590** — two Track-F coverage clears (effort "med" tier, units BFS dead-end null). **LESSONS:** (1) **"Advertised-but-missing" is a real, high-value gap class on the SECONDARY platform.** A premium perk that the web ships and the paywall promises, but the native app can't reach, is buyer-disappointment / refund risk — closing it clears the value bar strongly (monetization/parity), not padding. When scouting mobile, cross-check every `PREMIUM_PERKS`/paywall claim against an actual reachable mobile surface. (2) **The mobile route pattern is a reliable template.** A new mobile premium endpoint = clone the closest sibling (discover/plan/recipes-detail routes): `verifyMobileToken` → `rateLimit` → load signals → server-side `canUse` gate → per-user AI quota (only when `hasLlm`) → keyless-first core call → `serverError` catch. Import-path depth: a route one dir deeper than the sibling needs ONE extra `..` on every `_lib` import — typecheck catches it, but count them first. (3) **Load entitlement signals UNCONDITIONALLY, never behind `if (billingOn)`.** Reviewer A caught that gating `isPremium(signals)` on the billing flag silently downgrades a real premium user to the free AI quota when `FEATURE_BILLING` is off. The established pattern (plan/route.ts + the web remix page) loads signals regardless of the flag and uses `billingOn` ONLY on the `canUse` gate. (4) **A new client screen that fires per-selection fetches needs a staleness guard.** The remix screen re-fetches on each axis-tab tap; without a monotonic request-gen ref (or an axis-match check) before committing state, out-of-order resolution shows a mismatched result. `cook/[id].tsx`'s `cancelled` flag is the repo precedent — replicate it. (5) **A mobile Family-tier paywall card is STILL a dark-feature trap (re-confirmed, run 78 saw the store-doc mirror):** Family is behind `FEATURE_HOUSEHOLDS` (off); the web gates its Family card behind the flag, so an always-visible mobile Family card would advertise a feature a new user can't reach. **Readiness:** did NOT open the 'ready' issue — sole DoD gap unchanged (reach-gated floor #190, base ≈ $33K < $100K = owner-GTM). Confidence statement stays UNCHECKED. Security/RLS/Track-G re-confirmed CLEAN.
