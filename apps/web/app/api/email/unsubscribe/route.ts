 ...
 * Both arguments are HTML-escaped at the interpolation point so the helper is safe even if a future
 * caller passes non-literal content (today all callers pass static strings).
 */
function page(title: string, message: string, status = 200): Response {
  const t = esc(title);
  const body = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${t}</title></head>
<body style="margin:0;background:#f6f7f8;font:400 16px/1.6 -apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1d2530;">
  <div style="max-width:440px;margin:64px auto;padding:28px;background:#fff;border:1px solid #e6e8ea;border-radius:14px;">
    ...
    <h1 style="margin:0 0 10px;font:700 20px/1.3 inherit;">${t}</h1>
    <p style="margin:0;color:#525d6a;">${esc(message)}</p>
  </div>
</body></html>`;
