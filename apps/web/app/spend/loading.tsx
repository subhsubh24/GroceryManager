      ...
      <div className="mt-6 space-y-6">
        ...
        <section className="grid grid-cols-2 gap-4">
          ...
          <div className="card-pad space-y-2">
            ...
            <div className="h-3 w-32 animate-pulse rounded bg-surface-1" />
          </div>
        </section>

        {/* Top items by spend */}
        <section className="card-pad">
          <div className="mb-3 h-4 w-36 animate-pulse rounded bg-surface-1" />
          <ul className="space-y-1">
            {Array.from({ length: 8 }).map((_, i) => (
              <li key={i} className="flex justify-between">
                <div className="h-4 w-40 animate-pulse rounded bg-surface-1" />
                <div className="h-4 w-14 animate-pulse rounded bg-surface-1" />
              </li>
            ))}
          </ul>
        </section>
        ...
        {/* Cheaper elsewhere */}
        <section className="card-pad">
          <div className="mb-3 h-4 w-32 animate-pulse rounded bg-surface-1" />
          <ul className="space-y-2">
            {Array.from({ length: 5 }).map((_, i) => (
              <li key={i} className="h-4 w-5/6 animate-pulse rounded bg-surface-1" />
            ))}
          </ul>
        </section>
      </div>
